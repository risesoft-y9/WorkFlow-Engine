        ...
        try {
            ...
            if (itembox.equalsIgnoreCase(ItemBoxTypeEnum.DRAFT.getValue()) || itembox.equalsIgnoreCase(ItemBoxTypeEnum.ADD.getValue())) {
                if (list.size() >= 1) {
                    ...
                    if (!addable) {
                        // 没有意见框编辑权限时，增加代录权限
                        boolean hasRole1 = roleManager.hasRole(Y9LoginUserHolder.getTenantId(), "itemAdmin", "", "代录意见角色", person.getPersonId());
                        if (hasRole1) {
                ...
                if (StringUtils.isNotBlank(taskId)) {
                    boolean hasRole = roleManager.hasRole(Y9LoginUserHolder.getTenantId(), "itemAdmin", "", "代录意见角色", person.getPersonId());
                    if (hasRole) {
                        // 没有意见框编辑权限时，增加代录权限
                        Boolean addable = (Boolean)addableMap.get("addable");
                        if (!addable) {
    ...
    public Opinion saveOrUpdate(Opinion entity) {
        ...
        if (StringUtils.isBlank(id)) {
            ...
            if (isAgent == 1) {
                ...
                OrgUnit orgUnit1 = orgUnitManager.getOrgUnit(tenantId, position.getParentId());
        ...
        OrgUnit orgUnit0 = orgUnitManager.getOrgUnit(tenantId, position.getParentId());
