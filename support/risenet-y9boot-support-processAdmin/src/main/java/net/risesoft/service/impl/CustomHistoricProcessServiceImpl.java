...
public class CustomHistoricProcessServiceImpl implements CustomHistoricProcessService {
    ...
    public boolean deleteProcessInstance(String processInstanceId) {
        try {
            ...
            if (his.getEndTime() == null) {
                ...
                String updateSql = "UPDATE act_hi_procinst t SET t.DELETE_REASON_ = #{DELETE_REASON_} WHERE t.PROC_INST_ID_=#{processInstanceId}";
                historyService.createNativeHistoricProcessInstanceQuery().sql(updateSql).parameter("DELETE_REASON_", "已删除").parameter("processInstanceId", processInstanceId).singleResult();
    ...
    public List<HistoricProcessInstance> deleteProList(String itemId, Integer page, Integer rows) {
        return historyService.createHistoricProcessInstanceQuery().variableValueEquals("itemId", itemId).deleted().orderByProcessInstanceStartTime().desc().listPage((page - 1) * rows, rows);
    }
    ...
    public HistoricProcessInstance getById(String processInstanceId, String year) {
        if (StringUtils.isNotBlank(year)) {
            String sql = "SELECT * from ACT_HI_PROCINST_" + year + " p where p.PROC_INST_ID_ = '" + processInstanceId + "'";
    ...
    public List<HistoricProcessInstance> getBySuperProcessInstanceId(String superProcessInstanceId) {
        return historyService.createHistoricProcessInstanceQuery().superProcessInstanceId(superProcessInstanceId).notDeleted().orderByProcessInstanceStartTime().asc().list();
    }

    @Override
    public List<HistoricProcessInstance> getRecycleAll(String title, Integer page, Integer rows) {
        return historyService.createHistoricProcessInstanceQuery().deleted().variableValueLike(SysVariables.DOCUMENTTITLE, "%" + title + "%").orderByProcessInstanceStartTime().desc().listPage((page - 1) * rows, rows);
    }

    @Override
    public List<HistoricProcessInstance> getRecycleByItemId(String itemId, String title, Integer page, Integer rows) {
        return historyService.createHistoricProcessInstanceQuery().variableValueEquals("itemId", itemId).deleted().variableValueLike(SysVariables.DOCUMENTTITLE, "%" + title + "%").orderByProcessInstanceStartTime().desc().listPage((page - 1) * rows, rows);
    }

    @Override
    public List<HistoricProcessInstance> getRecycleByUserId(String title, String userId, Integer page, Integer rows) {
        return historyService.createHistoricProcessInstanceQuery().involvedUser(userId).deleted().variableValueLike(SysVariables.DOCUMENTTITLE, "%" + title + "%").orderByProcessInstanceStartTime().desc().listPage((page - 1) * rows, rows);
    }

    @Override
    public int getRecycleCount(String title) {
        int count = (int)historyService.createHistoricProcessInstanceQuery().variableValueLike(SysVariables.DOCUMENTTITLE, "%" + title + "%").or().deleted().count();
        return count;
    }

    @Override
    public int getRecycleCountByItemId(String itemId, String title) {
        int count = (int)historyService.createHistoricProcessInstanceQuery().variableValueEquals("itemId", itemId).variableValueLike(SysVariables.DOCUMENTTITLE, "%" + title + "%").or().deleted().count();
        return count;
    }

    @Override
    public int getRecycleCountByUserId(String title, String userId) {
        int count = (int)historyService.createHistoricProcessInstanceQuery().involvedUser(userId).variableValueLike(SysVariables.DOCUMENTTITLE, "%" + title + "%").or().deleted().count();
        return count;
    }
    ...
    public boolean recoveryProcessInstance(String processInstanceId) {
        try {
            ...
            if (his.getEndTime() == null) {
                ...
                String updateSql = "UPDATE act_hi_procinst t SET t.DELETE_REASON_ = #{DELETE_REASON_,jdbcType=VARCHAR} WHERE t.PROC_INST_ID_=#{processInstanceId}";
                historyService.createNativeHistoricProcessInstanceQuery().sql(updateSql).parameter("DELETE_REASON_", null).parameter("processInstanceId", processInstanceId).singleResult();
            } else {
                // 办结件恢复
                String updateSql = "UPDATE act_hi_procinst t SET t.DELETE_REASON_ = #{DELETE_REASON_,jdbcType=VARCHAR} WHERE t.PROC_INST_ID_=#{processInstanceId}";
                historyService.createNativeHistoricProcessInstanceQuery().sql(updateSql).parameter("DELETE_REASON_", null).parameter("processInstanceId", processInstanceId).singleResult();
    ...
    public boolean removeProcess(String processInstanceId) {
        try {
            ...
            if (his != null && his.getEndTime() == null) {
                ...
                // 数据中心办结件
                if (his == null) {
                    OfficeDoneInfoModel officeDoneInfoModel = officeDoneInfoApi.findByProcessInstanceId(tenantId, processInstanceId);
    ...
    public void setPriority(String processInstanceId, String priority) throws Exception {
        ...
        if (null != processInstance) {
            String executionsql = "UPDATE ACT_RU_EXECUTION T SET T.TENANT_ID_ = #{TENANT_ID_} WHERE T.ID_=#{processInstanceId}";
            runtimeService.createNativeProcessInstanceQuery().sql(executionsql).parameter("TENANT_ID_", priority).parameter("processInstanceId", processInstanceId).singleResult();
        }
        String updateSql = "UPDATE ACT_HI_PROCINST T SET T.TENANT_ID_ = #{TENANT_ID_} WHERE T.PROC_INST_ID_=#{processInstanceId}";
        runtimeService.createNativeProcessInstanceQuery().sql(updateSql).parameter("TENANT_ID_", priority).parameter("processInstanceId", processInstanceId).singleResult();
    }
