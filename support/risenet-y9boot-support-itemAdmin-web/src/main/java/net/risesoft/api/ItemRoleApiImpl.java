...
public class ItemRoleApiImpl implements ItemRoleApi {
    ...
    public List<Map<String, Object>> findCsUserBureau(String tenantId, String userId, Integer principalType) {
        ...
        if (principalType == ItemPrincipalTypeEnum.DEPT.getValue()) {
            OrgUnit orgunit = personManager.getBureau(tenantId, userId);
    ...
    public List<Map<String, Object>> getOrgTree(String tenantId, String userId, String id, String treeType, String name) {
        ...
        if (StringUtils.isNotBlank(name)) {
            orgUnitList = orgUnitManager.treeSearch(tenantId, name, treeType);
        } else {
            orgUnitList = orgUnitManager.getSubTree(tenantId, id, treeType);
        }
        for (int i = 0; i < orgUnitList.size(); i++) {
            ...
            } else if ("Person".equals(orgUnitList.get(i).getOrgType())) {
                person = personManager.getPerson(tenantId, orgUnitList.get(i).getId());
