...
public class OpinionServiceImpl implements OpinionService {
    ...
    public List<Map<String, Object>> personCommentList(String processSerialNumber, String taskId, String itembox, String opinionFrameMark, String itemId, String taskDefinitionKey, String activitiUser) {
        ...
        try {
            ...
            if (ItemBoxTypeEnum.DRAFT.getValue().equalsIgnoreCase(itembox) || ItemBoxTypeEnum.ADD.getValue().equalsIgnoreCase(itembox)) {
                if (list.size() >= 1) {
                    addableMap.put("addable", true);
                    ...
                    if (!b) {
                        // 没有意见框编辑权限时，增加代录权限
                        boolean hasRole1 = roleManager.hasRole(Y9LoginUserHolder.getTenantId(), "itemAdmin", "", "代录意见角色", userInfo.getPersonId());
                        if (hasRole1) {
                ...
                if (null != bind) {
                    ...
                    if (!roleIds.isEmpty()) {
                        for (String roleId : roleIds) {
                            Boolean hasRole = roleManager.hasRoleByTenantIdAndRoleIdAndOrgUnitId(tenantId, roleId, personId);
                            if (hasRole) {
                                addableMap.put("addable", true);
                                break;
                            }
                            ...
                            for (String roleId : roleIds) {
                                ...
                                if (ItemBoxTypeEnum.TODO.getValue().equalsIgnoreCase(itembox)) {
                                    ...
                                    if (isEntrust) {
                                         ...
                                         */
                                        hasRole = roleManager.hasRoleByTenantIdAndRoleIdAndOrgUnitId(tenantId, roleId, ownerId);
                                        if (hasRole) {
                                            addableMap.put("addable", true);
                                            continue;
                                        }
                                    } else {
                                        hasRole = roleManager.hasRoleByTenantIdAndRoleIdAndOrgUnitId(tenantId, roleId, personId);
                                        if (hasRole) {
                                            addableMap.put("addable", true);
                                            continue;
                                        }
                ...
                if (StringUtils.isNotBlank(taskId)) {
                    boolean hasRole = roleManager.hasRole(Y9LoginUserHolder.getTenantId(), "itemAdmin", "", "代录意见角色", userInfo.getPersonId());
                    if (hasRole) {
                        // 没有意见框编辑权限时，增加代录权限
            ...
            } else if (ItemBoxTypeEnum.DONE.getValue().equalsIgnoreCase(itembox) || ItemBoxTypeEnum.DOING.getValue().equalsIgnoreCase(itembox)) {
                addableMap.put("addable", false);
                for (Opinion opinion : list) {
                    Map<String, Object> map = new HashMap<String, Object>(16);
                    opinion.setContent(CommentUtil.replaceEnter2Br(opinion.getContent()));
    ...
    public Opinion saveOrUpdate(Opinion entity) {
        ...
        if (StringUtils.isBlank(id)) {
            ...
            asyncHandleService.sendMsgRemind(Y9LoginUserHolder.getTenantId(), Y9LoginUserHolder.getPersonId(), entity.getProcessSerialNumber(), entity.getContent());
            return o;
        }
