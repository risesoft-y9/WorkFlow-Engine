...
public class DocumentServiceImpl implements DocumentService {
    ...
    public String getFirstItem() {
        try {
            ...
            String tenantId = Y9LoginUserHolder.getTenantId(), userId = userInfo.getPersonId();
            String resourceId = "";
            List<Resource> list = personResourceApi.listSubResources(tenantId, userId, AuthorityEnum.BROWSE.getValue(), resourceId);
            String url = "";
            for (Resource r : list) {
                url = r.getUrl();
                    ...
                    if (!"发送".equals(buttonName)) {
                        ...
                        if (roleIds.isEmpty()) {
                            ...
                            for (String roleId : roleIds) {
                                boolean hasRole = roleManager.hasRoleByTenantIdAndRoleIdAndOrgUnitId(tenantId, roleId, userId);
                                if (hasRole) {
                                    Map<String, Object> mapTemp = new HashMap<String, Object>(16);
                                    mapTemp.put("menuName", buttonName);
                                    mapTemp.put("menuKey", buttonCustomId);
        ...
        try {
            ...
            if (SysVariables.COMMON.equals(multiInstance)) {
                ...
                if (itemTaskConf != null && itemTaskConf.getSignTask()) {
                    ...
                    List<OrgUnit> orgUnitList = this.getUserChoice(itemId, processDefinitionId, taskDefinitionKey, processParam != null ? processParam.getProcessInstanceId() : "");
                    // 只有一个人，则直接返回人员发送
                    if (orgUnitList.size() == 1 && OrgTypeEnum.PERSON.getEnName().equals(orgUnitList.get(0).getOrgType())) {
