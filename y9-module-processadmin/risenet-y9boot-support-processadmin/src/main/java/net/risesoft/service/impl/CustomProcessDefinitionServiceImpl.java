...
public class CustomProcessDefinitionServiceImpl implements CustomProcessDefinitionService {
    ...
    public List<Map<String, Object>> getFlowElement(String processDefinitionId, Boolean isContainStartNode) {
        ...
        for (FlowElement activity : activitieList) {
            ...
            if (activity.getName() != null && !activity.getName().isEmpty() && activity.getId() != null && !activity.getId().isEmpty()) {
                list.add(tempMap);
            }
        }
        Map<String, Object> tempMap = new LinkedHashMap<>();
    ...
    public List<Map<String, Object>> getNodes(String processDefinitionId, Boolean isContainStartNode) {
        ...
        if (!isContainStartNode) {
            ...
            activitieList.removeIf(e -> e instanceof Gateway || e instanceof StartEvent || e instanceof EndEvent || e instanceof SequenceFlow);
        } else {
            activitieList = getFilteredActivityImpls(processDefinitionId);
        }
        for (FlowElement activity : activitieList) {
