    ...
    public Y9Result<String> getHandleSerial(@RequestParam(required = true) String taskId) {
        try {
            ...
            for (Object obj : users) {
                ...
                Person employee = personApi.get(Y9LoginUserHolder.getTenantId(), user).getData();
        ...
        try {
            ...
            if (multiInstance.equals(SysVariables.COMMON)) {
                for (int i = 0; i < users.size(); i++) {
                    Map<String, Object> map = new HashMap<String, Object>(16);
                    Person employee = personApi.get(Y9LoginUserHolder.getTenantId(), users.get(i)).getData();
            ...
            if (multiInstance.equals(SysVariables.PARALLEL)) {
                ...
                for (int i = 0; i < htims.size(); i++) {
                    ...
                    if (b) {
                        Map<String, Object> map = new HashMap<String, Object>(16);
                        Person employee = personApi.get(tenantId, hai.getAssignee()).getData();
                        map.put("user", employee.getName());
