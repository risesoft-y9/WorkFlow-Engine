...
public class MainRestController {
    ...
    public Y9Result<Map<String, Object>> getCount4Item(String itemId, Model model) {
        ...
        try {
            ...
            if (itemModel != null && itemModel.getId() != null) {
                ...
                draftCount = draftManager.getDraftCount(Y9LoginUserHolder.getTenantId(), userInfo.getPersonId(), itemId);
                draftRecycleCount = draftManager.getDeleteDraftCount(Y9LoginUserHolder.getTenantId(), userInfo.getPersonId(), itemId);
                Map<String, Object> countMap = todoManager.getCountByUserIdAndProcessDefinitionKey(Y9LoginUserHolder.getTenantId(), userInfo.getPersonId(), processDefinitionKey);
    ...
    public Y9Result<Map<String, Object>> getItem(@RequestParam(required = true) String itemId) {
        ...
        boolean b = personRoleApi.hasRole(tenantId, "Y9OrgHierarchyManagement", "", "监控管理员角色", userInfo.getPersonId()).getData();
    ...
    public Map<String, Object> getMonitorAuthority(String itemId, Model model) {
        ...
        try {
            ...
            if (itemModel != null && itemModel.getId() != null) {
                model.addAttribute("processDefinitionKey", processDefinitionKey);
                boolean hasOnlineAccess = personRoleApi.hasRole(tenantId, Y9Context.getSystemName(), "", itemModel.getName() + "收件角色", userInfo.getPersonId()).getData();
            ...
            b = personRoleApi.hasRole(tenantId, "Y9OrgHierarchyManagement", "", "监控管理员角色", userInfo.getPersonId()).getData();
            if (b) {
