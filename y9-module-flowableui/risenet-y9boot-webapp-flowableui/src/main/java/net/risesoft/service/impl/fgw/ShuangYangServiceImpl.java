...
public class ShuangYangServiceImpl implements ShuangYangService {
    ...
    private static String getFwLinkLjSql(final PushData pushData) {
        String sql =
            "SELECT lw.hallreg from bpm_linkruntime l, d_gw_lwinfo lw WHERE lw.lwinfouid = l.toinstanceid AND l.frominstanceid = '"
                + pushData.getProcessinstanceid() + "' AND lw.hallreg IS NOT NULL AND l.linktype != 'BING_AN'";
        return sql;
    }

    // 查询是否是委托评估
    private static String getIswtpgSql(final PushData pushData) {
        String sql = "select * from y9_form_fw where wtpg = '1' AND GUID='" + pushData.getProcessserialnumber() + "'";
        return sql;
    }
    ...
    private static String getLjLinkFwSql(PushData pushData) {
        ...
        if (pushData != null && "办结".equals(pushData.getEventtype())) {
            sql =
                "SELECT lw.hallreg, lwpi.instanceid lwpiinstanceid, lwpi.bingan FROM bpm_processinstancedone lwpi, bpm_linkruntime l, d_gw_lwinfo lw WHERE lwpi.businesskey = lw.lwinfouid AND lwpi.instanceid = l.frominstanceid AND l.toinstanceid ='"
                    + pushData.getProcessinstanceid() + "' AND lw.hallreg IS NOT NULL AND l.linktype != 'BING_AN'";
        } else {
            sql =
                "SELECT lw.hallreg, lwpi.instanceid lwpiinstanceid, lwpi.bingan FROM bpm_processinstanceruntime lwpi, bpm_linkruntime l, d_gw_lwinfo lw WHERE lwpi.businesskey = lw.lwinfouid AND lwpi.instanceid = l.frominstanceid AND l.toinstanceid ='"
                    + pushData.getProcessinstanceid() + "' AND lw.hallreg IS NOT NULL AND l.linktype != 'BING_AN'";
        }
        return sql;
    }
    ...
    public Boolean pushDataInterface(PushData data) {
        ...
        try {
            ...
            xml += "<operateDate>" + sdf.format(data.getCreatedate() != null ? data.getCreatedate() : new Date())
                + "</operateDate>";
            xml += "</pushdata>";
