...
import net.risesoft.util.gfg.RedisUtil;
...
public class LinkServiceImpl implements LinkService {
    private static JdbcTemplate oldjdbcTemplate = OldUtil.getOldjdbcTemplate();
     ...
     */
    @Override
    public Y9Page<Map<String, Object>> findPiByProcessId(String processId, String fromInstanceId, String searchMapStr, Integer page, Integer row) {
        if (OldUtil.newFwProcessId.equals(processId)){
            ...
            String userId = Y9LoginUserHolder.getUserInfo().getCaid().replace("-","").replace("{","").replace("}","");
            ...
            StringBuffer sql_content = new StringBuffer();
            sql_content.append("select * from (");
            sql_content.append("select query.*, rownum rnum from (");
            sql_content.append(this.linkListSql(processId,userId,1,searchMapStr));
            sql_content.append(") query where rownum <= " + page * row);
            sql_content.append(") where rnum >" + (page -1) * row  + " order by rnum");
            List<Map<String, Object>> list = oldjdbcTemplate.queryForList(sql_content.toString());
            List<Map<String, Object>> dataList = new ArrayList();
    ...
    private String linkListSql(String processId, String userId,int tag,String searchMapStr) {
        ...
        //来文特殊处理
        if (!processId.equals("_HtSCIfOuEeSrt-rFRbhdxw")) {
        ...
        LOGGER.info("查询老系统可关联的关联文件的sql:- "+sb.toString());
        return sb.toString();
    }
    ...
    public void saveLink(String processInstanceId, List<GLJ> to) {
        for (GLJ glj : to) {
            String sql = "SELECT linkId FROM bpm_linkruntime a " +
                    "WHERE a.fromInstanceId = '"+processInstanceId+"' " +
                    "AND a.toInstanceId = '"+glj.getInstanceId()+"' " +
            ...
            String insertSql = "insert into bpm_linkruntime ( LINKID,DESCRIPTION,FROMINSTANCEID,NAME," +
                    "TOINSTANCEID,BPMSERVER,CREATED,CREATERDN," +
                    "CREATERNAME,CREATERUID,TENANTID,UPDATED," +
                    "UPDATERDN,UPDATERNAME,UPDATERUID,LINKACTIVITYID,LINKTYPE) " +
                    "values ('"+ Y9IdGenerator.genId() +"','"+glj.getTitle()+"','"+processInstanceId+"','"+glj.getSerialNumber()+"'," +
                    "'"+glj.getInstanceId()+"','new','"+time+"','"+user.getDn()+"'," +
                    "'"+user.getName()+"','"+userId+"','"+tenantId+"','"+time+"'," +
                    "'"+user.getDn()+"','"+user.getName()+"','"+userId+"','"+time+"'," +
                    "'NORMAL')";
...
//        List<LinkModel> results = oldjdbcTemplate.query(sql, new BeanPropertyRowMapper<LinkModel>(LinkModel.class));
        ...
        if ("NORMAL".equals(linkModel.getLinktype())){
            ...
            if (OldUtil.ljProcessId.equals(cxfrom.getProcessId()) && (OldUtil.fwProcessId.equals(cxto.getProcessId()) || OldUtil.newFwProcessId .equals(cxto.getProcessId()))) {
                ...
                if (cxList.contains(lwzbdept)) {//办结方式制空，办理方式不制空
                    ...
                    xgSql = " update D_GW_LWINFO set finishtype=0,handletype=0 where lwinfouid ='"+from.getBusinesskey()+"'";
                }
        ...
        lwInfoApi.delLwInfo(Y9LoginUserHolder.getTenantId(), linkModel.getFrominstanceid());
    }
    ...
    public Map<String, List<CXLink>> findByInstanceId(String processInstanceId) {
        //查询link表数据
        //作为关联0
//        List<LinkModel> fromlist = oldjdbcTemplate.query("select * from bpm_linkruntime a where a.fromInstanceId = '"+processInstanceId+"' order by a.created desc ",(rs, rowNum) -> {
//            LinkModel linkModel = new LinkModel();
//            linkModel.setLinkid(rs.getObject("LINKID") != null ? rs.getObject("LINKID").toString():"");
//            linkModel.setDescription(rs.getObject("DESCRIPTION") != null ? rs.getObject("DESCRIPTION").toString():"");
//            linkModel.setFrominstanceid(rs.getObject("FROMINSTANCEID") != null ? rs.getObject("FROMINSTANCEID").toString():"");
//            linkModel.setName(rs.getObject("NAME") != null ? rs.getObject("NAME").toString():"");
//            linkModel.setToinstanceid(rs.getObject("TOINSTANCEID") != null ? rs.getObject("TOINSTANCEID").toString():"");
//            linkModel.setBpmserver(rs.getObject("BPMSERVER") != null ? rs.getObject("BPMSERVER").toString():"");
//            linkModel.setCreated((Date) rs.getObject("CREATED"));
//            linkModel.setCreaterdn(rs.getObject("CREATERDN") != null ? rs.getObject("CREATERDN").toString():"");
//            linkModel.setCreatername(rs.getObject("CREATERNAME") != null ? rs.getObject("CREATERNAME").toString():"");
//            linkModel.setCreateruid(rs.getObject("CREATERUID") != null ? rs.getObject("CREATERUID").toString():"");
//            linkModel.setTenantid(rs.getObject("TENANTID") != null ? rs.getObject("TENANTID").toString():"");
//            linkModel.setUpdated((Date) rs.getObject("UPDATED"));
//            linkModel.setUpdaterdn(rs.getObject("UPDATERDN") != null ? rs.getObject("UPDATERDN").toString():"");
//            linkModel.setUpdatername(rs.getObject("UPDATERNAME") != null ? rs.getObject("UPDATERNAME").toString():"");
//            linkModel.setUpdateruid(rs.getObject("UPDATERUID") != null ? rs.getObject("UPDATERUID").toString():"");
//            linkModel.setLinktype(rs.getObject("LINKTYPE") != null ? rs.getObject("LINKTYPE").toString():"");
//            return linkModel;
//        });
        String fromSql = "select * from bpm_linkruntime a where a.fromInstanceId = '"+processInstanceId+"' order by a.created desc ";
        ...
        //作为被关联1
//        List<LinkModel> tolist = oldjdbcTemplate.query("select * from bpm_linkruntime a where a.toInstanceId = '"+processInstanceId+"' order by a.created desc ",(rs, rowNum) -> {
//            LinkModel linkModel = new LinkModel();
//            linkModel.setLinkid(rs.getObject("LINKID") != null ? rs.getObject("LINKID").toString():"");
//            linkModel.setDescription(rs.getObject("DESCRIPTION") != null ? rs.getObject("DESCRIPTION").toString():"");
//            linkModel.setFrominstanceid(rs.getObject("FROMINSTANCEID") != null ? rs.getObject("FROMINSTANCEID").toString():"");
...
//            return linkModel;
//        });
        String toSql = "select * from bpm_linkruntime a where a.toInstanceId = '"+processInstanceId+"' order by a.created desc ";
    ...
    private Map<String , List<CXLink>> linkMap(String instanceId, List<LinkModel> links, int listType) {
        ...
        for (LinkModel linkModel : links) {
            ...
            if ((to != null || nto != null) && (from != null || nfrom != null)){
                if (from != null){
                    ccFrom = from;
                }
                if (nfrom != null){
            ...
            for (ProcessModel process : processList) {
                ...
                if (listType == 0 && (to != null || nto != null) ){
                    if (to != null){
                ...
                if (ccFrom != null && ccTo != null){
                    ...
                    if ("0".equals(CXLink.getType())){
                        if (StringUtils.isNotBlank(activityInstanceId)){
                            CXLink.setLinkUrl("/bpm/client/instance_open?activityInstanceId="+activityInstanceId);
                        }
                    }else {
                        CXLink.setLinkUrl("flowable");
                    }
        ...
        for (String processName : cxmap.keySet()) {
            ...
            for (CXLink CXLink : CXLinks) {
                ...
                if(from.getBingan() == 1 && listType == 1){
                    String baSql = "select * from bpm_linkruntime a where a.fromInstanceId = '"+from.getInstanceid()+"' order by a.created desc ";
//                    List<LinkModel> fromlist = oldjdbcTemplate.query(baSql,(rs, rowNum) -> {
//                        LinkModel linkModel = new LinkModel();
//                        linkModel.setLinkid(rs.getObject("LINKID") != null ? rs.getObject("LINKID").toString():"");
//                        linkModel.setDescription(rs.getObject("DESCRIPTION") != null ? rs.getObject("DESCRIPTION").toString():"");
//                        linkModel.setFrominstanceid(rs.getObject("FROMINSTANCEID") != null ? rs.getObject("FROMINSTANCEID").toString():"");
...
//                        return linkModel;
//                    });
                    List<LinkModel> fromlist = oldjdbcTemplate.query(baSql, new BeanPropertyRowMapper<>(LinkModel.class) );
        ...
        try {
            if (StringUtils.isNotBlank(instanceId)){
                ...
                if (pm != null){
                    ...
                    String lshSql = "select lsh from y9_form_fw where guid = '" + pm.getProcessSerialNumber() + "'";
                    List<String> res = jdbcTemplate.query(lshSql, (rs, rowNum) -> rs.getString("lsh"));
                    String lsh = res.isEmpty() ? null : res.get(0);
    ...
    private String getBmjc (String personDn){
        ...
        if(dns.length == 1){
            ...
            departmentName = departmentName.substring(2,departmentName.length());
            return null;
        }else{
            departmentName = dns[dns.length - 2];
            dmDN = dns[dns.length - 2] + "," + dns[dns.length - 1];
            departmentName = departmentName.substring(3,departmentName.length());
        }
