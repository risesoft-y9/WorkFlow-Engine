...
public class ProcessInstanceVueController {
	...
	/**
	 * 删除流程实例 现在有两种情况要删除流程实例： 1.用户直接删除：此时只需有processInstanceId即可，不需要reason
	 * 2.用户拒收：此时不止需有processInstanceId，还需填写拒收原因
	 *
			...
			try {
				...
				if (StringUtils.isNotBlank(processInstance.getStartUserId())) {
					...
					if (userIdAndDeptId.length == 1) {
						...
						position = positionApi.getPosition(tenantId, userIdAndDeptId[0]);
						if (null != position) {
							orgUnitApi.getOrgUnit(tenantId, processInstance.getStartUserId().split(":")[1]);
