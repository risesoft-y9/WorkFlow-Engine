 ...
 * @LastEditTime: 2026-06-01 14:58:01
 * @Descripttion: 退回
...
<template>
    ...
    >
        <el-divider v-if="optType != 'back2any' && optType != 'back2draft'" content-position="left">{{
            $t('任务列表')
        }}</el-divider>
...
<script lang="ts" setup>
    ...
    let { title, loading, multiInstance, reason, tableConfig } = toRefs(data);
    ...
    function show() {
        ...
        if (props.optType != 'back2any' && props.optType != 'back2draft') {
            //多步退回不获取任务列表
    ...
    function submit() {
        if (reason.value == '') {
            ElMessage({
                type: 'error',
                message: t('请输入') + title.value + t('原因'),
                offset: 65,
                appendTo: '.task-list'
            });
            return;
        ...
        if (props.optType == 'rollback') {
            ...
            buttonApi
                .rollback2history(
                    ...
                    props.rollbackInfo.key,
                    props.rollbackInfo.orgUnitIds.toString(),
                    reason.value
                )
                .then((res) => {
                    loading.value = false;
