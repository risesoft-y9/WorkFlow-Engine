...
<div class="buttonManage">
  <y9Card :showHeader="false">
    <el-form ref="commonForm" style="height: 100%;" :model="commonFormData" :rules="rules">
      <y9Table :config="commonTableConfig" :filterConfig="commonFilterConfig">
        <template #common_btn>
          <el-button class="global-btn-main" type="primary" @click="addButton('common')"><i class="ri-add-line"></i>普通按钮</el-button>
        </template>
        <template #cname="{row,column,index}">
            <el-form-item prop="name" v-if="editIndex_common === index">
                <el-input  v-model="commonFormData.name" clearable/>
            </el-form-item>
            <span v-else>{{row.name}}</span>
        </template>
        <template #c_customId="{row,column,index}">
            <el-form-item prop="customId" v-if="editIndex_common === index">
                <el-input ref="customSign_c" :disabled="isEdit" v-model="commonFormData.customId" clearable/>
            </el-form-item>
            <span v-else>{{row.customId}}</span>
        </template>
        <template #common_opt="{row,column,index}">
          <div v-if="editIndex_common === index">
            <el-button class="global-btn-second" size="small" @click="saveData('common',commonForm)"><i class="ri-book-mark-line"></i>保存</el-button>
            <el-button class="global-btn-second" size="small" @click="cancalData('common',commonForm)"><i class="ri-close-line"></i>取消</el-button>
          </div>
          <div v-else>
            <el-button class="global-btn-second" size="small" @click="bindDetail(row)"><i class="ri-book-3-line"></i>绑定详情</el-button>
            <el-button class="global-btn-second" size="small" @click="editButton('common',row,index)"><i class="ri-edit-line"></i>修改</el-button>
            <el-button class="global-btn-danger" type="danger" size="small" @click="delButton('common',row)"><i class="ri-delete-bin-line"></i>删除</el-button>
          </div>
        </template>
      </y9Table>
    </el-form>
  ...
  <y9Card :showHeader="false">
    <div class="sendList">
      <el-form ref="sendForm" style="height: 100%;" :model="sendFormData" :rules="rules">
        <y9Table :config="sendTableConfig" :filterConfig="sendFilterConfig">
          <template #send_btn>
            <el-button class="global-btn-main" type="primary" @click="addButton('send')"><i class="ri-add-line"></i>发送按钮</el-button>
          </template>
          <template #sname="{row,column,index}">
              <el-form-item prop="name" v-if="editIndex_send === index">
                <el-input  v-model="sendFormData.name" clearable/>
              </el-form-item>
              <span v-else>{{row.name}}</span>
          </template>
          <template #s_customId="{row,column,index}">
              <el-form-item prop="customId" v-if="editIndex_send === index">
                      <el-input ref="customSign_s" :disabled="isEdit" v-model="sendFormData.customId" clearable/>
              </el-form-item>
              <span v-else>{{row.customId}}</span>
          </template>
          <template #send_opt="{row,column,index}">
            <div v-if="editIndex_send === index">
              <el-button class="global-btn-second" size="small" @click="saveData('send',sendForm)"><i class="ri-book-mark-line"></i>保存</el-button>
              <el-button class="global-btn-second" size="small" @click="cancalData('send',sendForm)"><i class="ri-close-line"></i>取消</el-button>
          </div>
          <div v-else>
            <el-button class="global-btn-second" size="small" @click="bindDetail(row)"><i class="ri-book-3-line"></i>绑定详情</el-button>
            <el-button class="global-btn-second" size="small" @click="editButton('send',row,index)"><i class="ri-edit-line"></i>修改</el-button>
            <el-button class="global-btn-danger" type="danger" size="small" @click="delButton('send',row)"><i class="ri-delete-bin-line"></i>删除</el-button>
          </div>
          </template>
        </y9Table>
      </el-form>
...
import {getCommonButtonList,saveOrUpdate,removeCommonButton,checkCustomId} from '@/api/itemAdmin/commonButton';
import {getSendButtonList,saveSendButton,removeSendButton,checkSendCustomId} from '@/api/itemAdmin/sendButton';
