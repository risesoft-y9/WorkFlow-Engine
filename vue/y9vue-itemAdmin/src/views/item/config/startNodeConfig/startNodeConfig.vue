<template>
  <y9Card :title="`启动路由配置${currInfo.name ? ' - ' + currInfo.name : ''}`">
		<div class="margin-bottom-20" v-if="Object.keys(currTreeNodeInfo).length > 0 && currTreeNodeInfo.systemName != ''">
			<el-row :gutter="40">
				...
				<el-col :span="17" style="padding-left: 0;">
					<el-tooltip placement="right" effect="customized" content="优先级越高，用户启动流程的时候就会优先判断是否有从该节点启动流程的权限。如果所有节点都没有权限，则以优先级最低的节点启动流程">
		...
		<el-drawer class="eldrawer" v-model="treeDrawer" direction='rtl' :title="title">
		...
		<el-drawer class="eldrawer" v-model="roleDrawer" direction='rtl' :title="title" @close="closeRoleTable">
...
<script lang="ts" setup>
  ...
  import {getBpmList,copyBind,bindRole,removeRole,getRoleList,getNodeList,saveOrder} from "@/api/itemAdmin/item/startNodeConfig";
  ...
  async function getStartNodeList(){//权限配置
  ...
  const moveUp = () => {//上移
      if(currentRow.value.length == 0){
		ElNotification({title: '操作提示',message: '请点击选择一条数据',type: 'error',duration: 2000,offset: 80});
        return;
      }
      let index = 0;
...
const moveDown = () => {//下移
  ...
  async function saveRoleBind() {
	...
	let result = await bindRole(props.currTreeNodeInfo.id,props.currTreeNodeInfo.processDefinitionId,taskDefKey.value,rIds.join(';'));
  ...
  async function getRoleTable(){
		let res = await getRoleList(props.currTreeNodeInfo.id,props.currTreeNodeInfo.processDefinitionId,taskDefKey.value);
			if(res.success){
	...
	if(row.roleIds.length==1){
		ElMessageBox.confirm(
			...
			let result = {success:false,msg:''};
				result = await removeRole(props.currTreeNodeInfo.id,props.currTreeNodeInfo.processDefinitionId,taskDefKey.value,row.roleIds.toString());
				ElNotification({
  ...
  async function delRoleBind() {
	...
	ElMessageBox.confirm(
			...
			let result = {success:false,msg:''};
			result = await removeRole(props.currTreeNodeInfo.id,props.currTreeNodeInfo.processDefinitionId,taskDefKey.value,roleIdArr.value.toString());
			ElNotification({
				title: result.success ? '成功' : '失败',
				message: result.msg,
				type: result.success ? 'success' : 'error',
