...
<script lang="ts" setup>
  ...
  import {getBindList,deleteBind,saveBind,copyPerm,removePerm} from "@/api/itemAdmin/item/permConfig";
  import {getRole,getRoleById,dynamicRole,getOrgList,getOrgTree,treeSearch} from "@/api/itemAdmin/item/permConfig";
  const props = defineProps({
	...
	async function reloadBindList(){//获取权限列表
	...
	async function savePermBind() {
		if(treeSelectedData.value.length == 0){
			ElNotification({title: '提示',message: '请选择'+title.value,type: 'info',duration: 2000,offset: 80});
			reject();
			return;
		}
		let roleIds = [];
		...
		let res = await saveBind(props.currTreeNodeInfo.id,props.processDefinitionId,props.taskDefKey,roleIds.join(';'),roleType.value);
