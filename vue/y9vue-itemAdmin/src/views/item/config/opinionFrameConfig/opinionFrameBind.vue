		...
		<y9Table :config="oFBListTableConfig" @select="handlerSelectData" @select-all="handlerSelectData"></y9Table>
		 <el-drawer v-model="tableDrawer" direction='rtl' :title="title">
     		<div style="margin: 8px 0;text-align: left;">
				<el-input style="width:200px;margin-right: 5px;" v-model="opinionFrameName" placeholder="意见框名称" clearable></el-input>
            ...
            @on-page-size-change="onPageSizeChange"></y9Table>
...
<script lang="ts" setup>
  ...
  import {getBpmList,getBindList,getOpinionFrameList,searchOpinionFrame,bindOpinionFrame,bindRole,removeOpinionFrame,removeRole,getRoleList,changeSignOpinion} from "@/api/itemAdmin/item/opinionFrameConfig";
  ...
  const props = defineProps({
	...
	const data = reactive({
		...
		oFBListTableConfig: {
			columns: [
				...
				{ title: "必签意见",key: "signOpinion",width: '100',
					render: (row) => {
						return h('button',{
	...
	function getOpinionFrameLists(){
		...
		searchOpinionFrame(props.currTreeNodeInfo.id,props.processDefinitionId,props.taskDefKey,opinionFrameName.value,page,rows).then(res =>{
			if(res.success){
	...
	async function saveOFBind() {
		...
		let res = await bindOpinionFrame(props.currTreeNodeInfo.id,props.processDefinitionId,props.taskDefKey,marks.join(';'));
		ElNotification({title: '操作提示',message: res.msg,type: res.success?'success':'error',duration: 2000,offset: 80});
