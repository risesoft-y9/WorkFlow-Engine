<template>
	<div class="organWordBindDiv">
		...
		<y9Table :config="buttonBindTableConfig" @select="handlerSelectData" @select-all="handlerSelectData" highlight-current-row></y9Table>
		 <el-drawer v-model="tableDrawer" direction='rtl' :title="title">
			<y9Table 
			:config="buttonTableConfig" 
...
<script lang="ts" setup>
  ...
  import {getBindList,getButtonList,removeButton,removeRole,bindRole,getRoleList,saveBindButton} from "@/api/itemAdmin/item/buttonConfig";
  ...
  const props = defineProps({
	...
	async function reloadBindList(){
		...
		roleIdArr.value = [];
		buttonArr.value = [];
		let res = await getBindList(props.currTreeNodeInfo.id,props.processDefinitionId,props.taskDefKey,props.buttonType);
		if(res.success){
	...
	function getButtonTable(){
		buttonArr.value = [];
		getButtonList(props.currTreeNodeInfo.id,props.processDefinitionId,props.taskDefKey,props.buttonType).then(res =>{
			if(res.success){
	...
	async function saveBind() {
		...
		if(buttonArr.value.length > 1){
			ElNotification({title: '操作提示',message: '只能勾选一条数据'+title.value,type: 'error',duration: 2000,offset: 80});
			return;
		}
		
		let res = await saveBindButton(props.currTreeNodeInfo.id,props.processDefinitionId,props.taskDefKey,buttonArr.value[0].id,props.buttonType);
