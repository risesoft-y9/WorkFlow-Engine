<template>
  <y9Card :title="`按钮配置${currInfo.name ? ' - ' + currInfo.name : ''}`">
		...
		<y9Table :config="buttonTableConfig" v-model:selectedVal="userSelectedData">
			<template #opt_button="{row, column, index}">
				<span style="margin-right:15px;" @click="buttonBindManage(row,1)"><i class="ri-add-line"></i>普通按钮</span>
				<span @click="buttonBindManage(row,2)"><i class="ri-add-line"></i>发送按钮</span>
			</template>
		</y9Table>

		<y9Dialog v-model:config="butcDialogConfig">
      		<buttonBind ref="buttonBindRef" :currTreeNodeInfo="currTreeNodeInfo" :processDefinitionId="currTreeNodeInfo.processDefinitionId" :taskDefKey="taskDefKey" :buttonType="buttonType"/>
		</y9Dialog>
	</y9Card>
</template> 
...
<script lang="ts" setup>
  ...
  import {getBpmList,copyBind} from "@/api/itemAdmin/item/buttonConfig";
  ...
  async function getButtonList(){//权限配置
