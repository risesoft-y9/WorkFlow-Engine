...
-->
<template>
	<el-form ref="mappingForm" :model="mappingConf" :rules="rules" :inline-message="true" :status-icon="true" label-width="120px">
		<el-form-item prop="tableName" label="数据库表">
			<el-select v-model="mappingConf.tableName" placeholder="请选择" @change="tableChange">
				<el-option v-for="table in tableList" :key="table.id" :label="table.tableName+'('+table.tableCnName+')'" :value="table.tableName">
				</el-option>
			</el-select>
		</el-form-item>
		...
		<el-form-item v-if="activeName == 'item'" prop="mappingTableName" label="映射数据库表">
			<el-select v-model="mappingConf.mappingTableName" placeholder="请选择" @change="mappingTableChange">
				<el-option v-for="table in mappingTableList" :key="table.id" :label="table.tableName+'('+table.tableCnName+')'" :value="table.tableName">
				</el-option>
			</el-select>
		</el-form-item>
		<el-form-item prop="mappingName" label="映射字段">
			<el-select v-if="activeName == 'item'" v-model="mappingConf.mappingName" placeholder="请选择">
				<el-option v-for="column in mappingColumnList" :key="column.id" :label="column.fieldName+'('+column.fieldCnName+')'" :value="column.fieldName">
				</el-option>
			</el-select>
...
import {getConfInfo,getColumns} from "@/api/itemAdmin/item/mappingConfig";
...
function getInfo(){
	getConfInfo(props.id,currInfo.value.id,props.activeName == 'item' ? currInfo.value.dockingItemId : "").then(res => {
