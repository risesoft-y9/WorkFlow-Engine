 ...
 * @LastEditTime: 2025-12-30 14:33:37
 * @Descripttion: 编号绑定
 * @FilePath: \y9-vue\y9vue-itemAdmin\src\views\item\config\organWordConfig\organWordBind.vue
-->
<template>
    <div class="organWordBindDiv">
        ...
        <el-drawer v-model="tableDrawer" :title="title" direction="rtl">
            <y9Table :config="oFTableConfig" @select="handlerGetData" @select-all="handlerGetData"></y9Table>
...
<script lang="ts" setup>
    ...
    import {
        ...
        removeRole,
        saveBind
    } from '@/api/itemAdmin/item/organWordConfig';
    ...
    function delOrganWord() {
        ...
        ElMessageBox.confirm('你确定要删除绑定的编号吗？', '提示', {
    ...
    async function saveOFBind() {
        ...
        if (organWordArr.value.length > 1) {
            ElNotification({
                title: '操作提示',
                message: '只能勾选一条编号数据' + title.value,
                type: 'error',
                duration: 2000,
                offset: 80
            });
