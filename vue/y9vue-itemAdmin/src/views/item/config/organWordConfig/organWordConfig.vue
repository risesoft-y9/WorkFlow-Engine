<template>
  <y9Card :title="`编号配置${currInfo.name ? ' - ' + currInfo.name : ''}`">
    	...
    	<y9Dialog v-model:config="owDialogConfig">
     		 <organWordBind ref="organWordBindRef" :currTreeNodeInfo="currTreeNodeInfo" :processDefinitionId="currTreeNodeInfo.processDefinitionId" :taskDefKey="taskDefKey"/>
		</y9Dialog>
	</y9Card>
</template>
...
<script lang="ts" setup>
  ...
  import {getBpmList,copyBind} from "@/api/itemAdmin/item/organWordConfig";
  const props = defineProps({
	...
	const data = reactive({
		...
		organWordListTableConfig: {//人员列表表格配置
			columns: [
				{
					title: "序号",
					type:'index',
  ...
  async function openOrganWordBind(row) {
    taskDefKey.value = row.taskDefKey;
		Object.assign(owDialogConfig.value,{
			show:true,
			width:'50%',
			title:'编号权限管理【'+row.taskDefName+'】',
			showFooter:false
		});
  }
