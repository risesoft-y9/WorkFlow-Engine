<template>
  <y9Card :title="`视图配置${currInfo.name ? ' - ' + currInfo.name : ''}`">
		<el-tabs v-model="activeName" style="height:40px" @tab-click="tabclick">
			<el-tab-pane label="草稿箱" name="draft" mark="11111"></el-tab-pane>
			...
			<el-tab-pane v-for="viewType in viewTypeList" :key="viewType.mark" :label="viewType.name" :name="viewType.mark"></el-tab-pane>
		</el-tabs>
		<div style="margin: 8px 0;">
			<el-row :gutter="40">
				<el-col :span="19">
					<el-button-group>
					<el-button type="primary" size="small" @click="addView('table')"><i class="ri-add-line"></i>新增</el-button>
					<el-button type="primary" size="small" @click="delView"><i class="ri-delete-bin-line"></i>删除</el-button>
					<el-button type="primary" size="small" @click="addView('custom')"><i class="ri-quill-pen-fill"></i>自定义列</el-button>
					<el-button type="primary" size="small" @click="copyViewData"><i class="ri-copyright-line"></i>复制</el-button>
				</el-button-group>
				</el-col>
				<el-col :span="5">
					<el-button-group>
						<el-button type="primary" size="small" @click="moveUp"><i class="ri-arrow-up-line"></i>上移</el-button>
						<el-button type="primary" size="small" @click="moveDown"><i class="ri-arrow-down-line"></i>下移</el-button>
						<el-button type="primary" @click="saveViewOrder"><span>保存</span></el-button>
					</el-button-group>
				</el-col>
			</el-row>
        </div>
		<y9Table v-model:selectedVal="userSelectedData" :config="viewTableConfig" @on-current-change="handleCurrentChange" @select-all="handlerSelectData" @select="handlerSelectData">
		...
		<y9Dialog v-model:config="vcDialogConfig">
      		<addOrEdit v-if="vcDialogConfig.type == 'addOrEdit'" ref="addOrEditRef" :currTreeNodeInfo="currTreeNodeInfo" :id="viewId" :optType="optType" :viewType="activeName"/>
			<copyView v-if="vcDialogConfig.type == 'copyView'" ref="copyViewRef" :currTreeNodeInfo="currTreeNodeInfo" :viewType="activeName" :vcDialogConfig="vcDialogConfig"/>
		</y9Dialog>
	</y9Card>
</template>
...
<script lang="ts" setup>
  ...
  import {getViewList,saveView,removeView,saveOrder,getViewTypeList} from "@/api/itemAdmin/item/viewConfig";
  ...
  async function getViewConfigList(){//权限配置
...
const moveUp = () => {//上移
      if(currentRow.value.length==0){
		ElNotification({title: '操作提示',message: '请点击选中一条数据',type: 'error',duration: 2000,offset: 80});
        return;
      }
      ...
      if (index > 0) {
		...
		ElNotification({title: '操作提示',message: '已经是第一条，不可上移',type: 'error',duration: 2000,offset: 80});
      }
}
...
const moveDown = () => {//下移
      ...
      if ((index + 1) == viewTableConfig.value.tableData.length) {
		ElNotification({title: '操作提示',message: '已经是最后一条，不可下移',type: 'error',duration: 2000,offset: 80});
      } else {
