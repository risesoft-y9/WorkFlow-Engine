...
-->
<template>
 <el-form class="tVariable" ref="tVariableForm" :model="formData" :rules="rules">
    <y9Table :config="varTableConfig" :filterConfig="filterConfig">
        <template #processModel>
          <label style="line-height: 32px;">选择任务</label>
          <el-select style="margin-left: 8px;" v-model="taskId" @change="taskChange">
            <el-option v-for="item in taskList" :key="item.taskId" :label="item.userName" :value="item.taskId">
            </el-option>
          </el-select>
				  <el-button :disabled="disabled" @click="addVariable" type="primary" style="margin-left: 10px;"><i class="ri-add-line" />新增</el-button>
		    </template>
      ...
      <template #opt_button="{row,column,index}">
        <div v-if="editIndex === index">
            <el-button class="global-btn-second" size="small" @click="saveData(tVariableForm)"><i class="ri-book-mark-line"></i>保存</el-button>
            <el-button class="global-btn-second" size="small" @click="cancalData(tVariableForm)"><i class="ri-close-line"></i>取消</el-button>
        </div>
        <div v-else>
          <el-button class="global-btn-second" size="small" :title="text" :disabled="disabled" @click="editTaskVariable(row,index)"><i class="ri-edit-line"></i>编辑</el-button>
          <el-button class="global-btn-second" size="small" :title="text" :disabled="disabled" @click="delTaskVariable(row)"><i class="ri-delete-bin-line"></i>删除</el-button>
        </div>
      </template>
    </y9Table>
  </el-form>
...
import {getTaskList,taskVarList,saveTaskVariable,deleteTaskVar} from '@/api/processAdmin/processControl';
