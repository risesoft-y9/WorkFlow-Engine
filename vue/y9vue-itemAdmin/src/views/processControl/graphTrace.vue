...
<template>
    <div class="containers" style="width: 100%;">
        <div>
            <div style="width: 15px;height: 15px;display: inline-block;background-color: #f3faf2;border: 1px solid green;margin-right: 5px;"></div>已经过的节点
            <div style="width: 15px;height: 15px;display: inline-block;background-color: #f9e8e9;border: 1px solid red;margin: 0 5px;"></div>当前节点
            <div style="width: 15px;height: 15px;display: inline-block;background-color: #eff1fa;border: 1px solid black;margin: 0 5px;"></div>未经过的节点
            <el-button style="float: right;" class="global-btn-second" size="small" @click="handleZoom(-0.2)">缩小</el-button>
            <el-button style="float: right;margin-right: 10px;" class="global-btn-second" size="small" @click="handleZoom(0.2)">放大</el-button>
        </div>
        <div class="canvas" style="height: 550px;width: 100%;" ref="canvas"></div>
    </div>
</template>
    ...
    try {
        ...
        if (res.success){
            ...
            if(res2.success){
                ...
                canvas1._activePlane.rootElement.children.forEach(item => {//处理UserTask
...
function genNodeDetailBox(detail,e,overlays){
    ...
    setTimeout(()=>{
        ...
        tempDiv.innerHTML = '<p><font>审批人员:</font>'+userName+'</p>'+
                            '<p><font>意见内容:</font>'+opinion+'</p>'+
                            '<p><font>开始时间:</font>'+startTime+'</p>'+
                            '<p><font>结束时间:</font>'+endTime+'</p>'+
                            '<p><font>审批耗时:</font>'+time+'</p>'+
                            '<p><font>节点描述:</font>'+deleteReason;
