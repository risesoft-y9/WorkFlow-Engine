...
-->
<template> 
<el-form class="pVariable" ref="pVariableForm" :model="formData" :rules="rules">
    <y9Table :config="varTableConfig" :filterConfig="filterConfig">
        <template #processModel>
				  <el-button :disabled="disabled" @click="addVariable" type="primary" style="margin-right: 10px;"><i class="ri-add-line" />新增</el-button>
		    </template>
        <template #varKey="{row,column,index}">
          <el-form-item prop="key" v-if="editIndex === index">
              <el-input v-model="formData.key" :disabled="editReadonly"></el-input>
          </el-form-item>
          <div v-else>{{row.key}}</div>
      </template>
        <template #renderVal="{row,column,index}">
          <el-form-item prop="value" v-if="editIndex === index">
      ...
      <template #opt_button="{row,column,index}">
        <div v-if="editIndex === index">
            <el-button class="global-btn-second" size="small" @click="saveData(pVariableForm)"><i class="ri-book-mark-line"></i>保存</el-button>
            <el-button class="global-btn-second" size="small" @click="cancalData(pVariableForm)"><i class="ri-close-line"></i>取消</el-button>
        </div>
        <div v-else>
          <el-button class="global-btn-second" size="small" :title="text" :disabled="disabled" @click="editProcessVariable(row,index)"><i class="ri-edit-line"></i>编辑</el-button>
          <el-button class="global-btn-second" size="small" :title="text" :disabled="disabled" @click="delProcessVariable(row)"><i class="ri-delete-bin-line"></i>删除</el-button>
        </div>
      </template>
    </y9Table>
  </el-form>
...
import {processVarList,saveProcessVariable,deleteProcessVar} from '@/api/processAdmin/processControl';
  ...
  refFrom.validate(valid => {
    if (valid) {
      const loading = ElLoading.service({ lock: true, text: '正在处理中', background: 'rgba(0, 0, 0, 0.3)' });
        saveProcessVariable(optType.value,props.processInstanceId,formData.value.key,formData.value.value).then(res => {
            loading.close();
            ElNotification({
