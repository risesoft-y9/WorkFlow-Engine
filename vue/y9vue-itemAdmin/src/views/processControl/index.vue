...
<template>
        <y9Table :config="controlTableConfig" :filterConfig="filterConfig"  @on-curr-page-change="onCurrPageChange"
            @on-page-size-change="onPageSizeChange">
            <template #processModel>
				<el-input v-model="processInstanceId" placeholder="流程实例ID" style="width:300px;margin-right: 10px;" clearable></el-input>
            ...
            <template #opt_button="{row,column,index}">
                ...
                <el-button class="global-btn-second" v-if="row.suspended" size="small" @click="active(row)">激活</el-button>
                <el-button class="global-btn-second" v-else size="small" @click="suspend(row)">挂起</el-button>
                <el-button class="global-btn-second" size="small" @click="delProcessInstance(row)">删除</el-button>
            </template>
        </y9Table>
    <y9Dialog v-model:config="dialogConfig">
        <GraphTraceNew v-if="dialogConfig.type == 'graphTraceNew'" ref="GraphTraceChild" :processDefinitionId="processDefinitionId" :processInstanceId="pInstanceId"/>
        <GraphTrace v-if="dialogConfig.type == 'graphTrace'" ref="GraphTraceChild" :processDefinitionId="processDefinitionId"/>
        <ProcessVariable v-if="dialogConfig.type == 'processVariable'" ref="ProcessVariableChild" :processInstanceId="pInstanceId" :suspended="suspended" />
        <TaskVariable v-if="dialogConfig.type == 'taskVariable'" ref="TaskVariableChild" :processInstanceId="pInstanceId" :suspended="suspended"/>
    </y9Dialog>
</template>
...
import {runningList,deleteProcessInstance,switchSuspendOrActive} from '@/api/processAdmin/processControl';
