        ...
        <el-form ref="optionClassForm" :model="formData" :rules="rules">
            <y9Table :config="tableConfig" :filterConfig="filterConfig">
                ...
                <template #type="{ row, column, index }">
                    <el-form-item v-if="editIndex === index" prop="type">
                        <el-input v-model="formData.type" :disabled="isEdit" clearable />
                    </el-form-item>
                    <span v-else>{{ row.type }}</span>
                </template>
                <template #opt="{ row, column, index }">
                    <div v-if="editIndex === index">
                        <el-button class="global-btn-second" size="small" @click="saveData(optionClassForm)"
                            ><i class="ri-book-mark-line"></i>保存
                        </el-button>
                        <el-button class="global-btn-second" size="small" @click="cancalData(optionClassForm)"
                            ><i class="ri-close-line"></i>取消
                        </el-button>
                    </div>
                    <div v-else>
                        <el-button class="global-btn-second" size="small" @click="editOptionValue(row)"
                            ><i class="ri-book-3-line"></i>字典管理
                        </el-button>
