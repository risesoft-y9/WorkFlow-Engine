  ...
  <el-form ref="optionValueForm" :model="formData" :rules="rules">
    <y9Table :config="tableConfig" @on-current-change="handleCurrentChange">
        ...
        <template #code="{row,column,index}">
            <el-form-item prop="code" v-if="editIndex === index">
                <el-input v-model="formData.code" clearable/>
            </el-form-item>
            <span v-else>{{row.code}}</span>
        </template>
        <template #defaultSelected="{row,column,index}">
          <i v-if="row.defaultSelected==1" class="ri-check-line" title="点击设置默认选中" @click="checkOption(row)" style="color:green;font-weight: bold;font-size: 22px;"></i>
          <i v-if="row.defaultSelected==0" class="ri-close-line" title="点击设置默认选中" @click="checkOption(row)" style="color:red;font-weight: bold;font-size: 22px;"></i>
        </template>
        <template #opt="{row,column,index}">
          <div v-if="editIndex === index">
              <el-button class="global-btn-second" size="small" @click="saveData(optionValueForm)"><i class="ri-book-mark-line"></i>保存</el-button>
              <el-button class="global-btn-second" size="small" @click="cancalData(optionValueForm)"><i class="ri-close-line"></i>取消</el-button>
          </div>
          <div v-else>
              <el-button class="global-btn-second" size="small" @click="editOptionValue(row,index)"><i class="ri-edit-line"></i>修改</el-button>
...
import {getOptionValueList,saveOptionValue,delOptionValue,updateOptionValue,saveOrder} from '@/api/itemAdmin/optionClass';
