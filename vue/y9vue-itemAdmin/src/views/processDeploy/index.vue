        ...
        <y9Table :config="deployTableConfig" :filterConfig="filterConfig">
            <template #deployProcess>
                ...
                >
                    ...
                    <font style="font-family: 微软雅黑;font-size: 14px;color: red;margin-left: 5px;">(文件格式xml/zip/bpmn)</font>
                </el-upload>
		    </template>
            ...
            <template #opt_button="{row,column,index}">
                <el-button class="global-btn-second" size="small" @click="showGraphTrace(row)"><i class="ri-image-line" title="流程图"></i>流程图</el-button>
                <el-button class="global-btn-second" size="small" @click="suspend(row)" v-if="!row.suspended"><i class="ri-pause-circle-line" title="挂起"></i>挂起</el-button>
                <el-button class="global-btn-second" size="small" @click="active(row)" v-else><i class="ri-play-circle-line" title="激活"></i>激活</el-button>
                <el-button class="global-btn-second" size="small" @click="delDeploy(row)"><i class="ri-delete-bin-line" title="删除"></i>删除</el-button>
            </template>
        </y9Table>
...
import {getDeployList,switchSuspendOrActive,deleteDeploy,deploy} from '@/api/processAdmin/processDeploy';
import type { UploadProps } from 'element-plus';

const data = reactive({
        processDefinitionId:'',
		actionUrl:import.meta.env.VUE_APP_CONTEXT + "vue/repository/deploy?access_token="+ y9_storage.getObjectItem(settings.siteTokenKey, 'access_token'),
		deployTableConfig: {//人员列表表格配置
			columns: [
...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
  if (rawFile.name.indexOf('.xml') == -1 && rawFile.name.indexOf('.zip') == -1 && rawFile.name.indexOf('.bpmn') == -1) {
...
function saveFile(params){//部署流程
    const loading = ElLoading.service({ lock: true, text: '正在处理中', background: 'rgba(0, 0, 0, 0.3)' });
    deploy(params.file).then(res => {
        loading.close();
        ElMessage({ type: res.success ? 'success' : 'error', message: res.msg, offset: 65 });
        if(res.success){
            getTableList();
