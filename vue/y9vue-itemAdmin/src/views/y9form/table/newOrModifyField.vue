...
-->
<template>
<el-form ref="fieldFormRef" class="newOrModifyField" :model="field" :rules="rules" :inline-message="true" :status-icon="true">
...
<script lang="ts" setup>
	...
	const props = defineProps({
		...
		fieldId:String,
		fieldList:Array,
		updateField: {//当前tree节点信息
			type: Object,
			default:() => { return null }
		},
		pushField:Function
	})
	var key = {"if":"if","else":"else","begin":"begin","end":"end","function":"function","add":"add","having":"having","exists":"exists","table":"table","insert":"insert","into":"into","in":"in","group":"group","delete":"delete","update":"update","field":"field","from":"from","drop":"drop","new":"new","like":"like","load":"load","to":"to"};
	...
	async function getFieldData(){
		...
		if(res.success){
			...
			if(props.fieldId != ""){//已保存的修改
				field.value = res.data.field;
				oldfieldName.value = res.data.field.fieldName;
				title.value = "修改业务表字段";
			}else if(props.updateField != null && props.fieldId == ""){//未保存的修改
	...
	async function saveOrModifyField(){
		await props.fieldList.forEach(element => {
			if(oldfieldName.value == null){
				if(element.fieldName == field.value.fieldName ){
					ElNotification({title: '失败',message: '字段英文名称已存在，请修改',type: 'error',duration: 2000,offset: 80});
					return;
				}
			}
		});
		if(props.tableId == ""){//新增
			field.value.state = 0;
			if(props.updateField != null){//未保存的修改
