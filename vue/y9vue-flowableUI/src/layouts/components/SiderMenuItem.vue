...
<template>
    <template v-if="!item.hidden">
            ...
            <el-sub-menu :index="item.name" class="y9-el-sub-menu">
                <template #title>
                    <template v-if="item.meta.icon">
                        <img v-if="item.meta.icon.indexOf('data:image/png;base64') > -1" :src="item.meta.icon" style="height:25px;vertical-align: middle;"/>
                        <i v-if="item.meta.icon.indexOf('data:image/png;base64') == -1" :class="['icon', item.meta.icon]" />
                    </template>
                    {{ $t(item.meta.title) }}
                    <div v-if="item.todoCount!=0">
                        <el-badge :value="item.todoCount" sty class="item submenu" type="danger"></el-badge>
                    </div>
                </template>
                <sider-menu-item
        ...
        <template v-else>
            <template v-if="item.name == 'email'">
                <a-link to="">
                    <el-menu-item :index="item.path" @click="openEmail()">
                        <template v-if="item.meta.icon">
                            <img v-if="item.meta.icon.indexOf('data:image/png;base64') > -1" :src="item.meta.icon" style="height:25px;vertical-align: middle;margin-right: 15px;margin-left: -5px;"/>
                            <img v-else-if="item.meta.icon.indexOf('assets') > -1" :src="getImageUrl(item.meta.icon.split('/')[1])" style="height:25px;vertical-align: middle;margin-right: 15px;margin-left: -5px;"/>
                            <i v-else-if="item.meta.icon.indexOf('data:image/png;base64') == -1" :class="['icon', item.meta.icon]" />
                        </template>
                        <template #title>
                            {{ $t(item.meta.title) }}
                            <div v-if="item.todoCount!=0">
            ...
            <template v-else>
                <a-link :to="item.path + '?itemId=' + ((item.path.indexOf('/workIndex') > -1 && item.itemId != undefined ) ? item.itemId : flowableStore.itemId)" @click="setAppType(item)">
                    <el-menu-item :index="item.path + (item.path.indexOf('/workIndex') > -1 ? item.itemId : '')" @click="toggleCollapsedFunc">
                        <template v-if="item.meta.icon">
                            <img v-if="item.meta.icon.indexOf('data:image/png;base64') > -1" :src="item.meta.icon" style="height:25px;vertical-align: middle;margin-right: 15px;margin-left: -5px;"/>
                            <img v-else-if="item.meta.icon.indexOf('assets') > -1" :src="getImageUrl(item.meta.icon.split('/')[1])" style="height:25px;vertical-align: middle;margin-right: 15px;margin-left: -5px;"/>
                            <i v-else-if="item.meta.icon.indexOf('data:image/png;base64') == -1" :class="['icon', item.meta.icon]" />
                        ...
                        <template #title>
                            ...
                            <div v-if="item.name=='draft'&& flowableStore.getDraftCount!=0">
                                <el-badge :value="flowableStore.getDraftCount" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='todo'&& flowableStore.getTodoCount!=0">
                                <el-badge :value="flowableStore.getTodoCount" class="item submenu" type="danger"></el-badge>
                            </div>
                            <div v-if="item.name=='doing'&& flowableStore.getDoingCount!=0">
                                <el-badge :value="flowableStore.getDoingCount" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='done'&& flowableStore.getDoneCount!=0">
                                <el-badge :value="flowableStore.getDoneCount" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='draftRecycle'&& flowableStore.getDraftRecycleCount!=0">
                                <el-badge :value="flowableStore.getDraftRecycleCount" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='monitorDoing'&& flowableStore.getMonitorDoing!=0">
                                <el-badge :value="flowableStore.getMonitorDoing" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='monitorDone'&& flowableStore.getMonitorDone!=0">
                                <el-badge :value="flowableStore.getMonitorDone" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='wtodo'&& flowableStore.getWtodoCount!=0">
                                <el-badge :value="flowableStore.getWtodoCount" class="item submenu" type="primary"></el-badge>
                            </div>
                            <div v-if="item.name=='wdone'&& flowableStore.getWdoneCount!=0">
                                <el-badge :value="flowableStore.getWdoneCount" class="item submenu" type="primary"></el-badge>
                            </div>
...
<script lang="ts">
import { defineComponent, PropType, toRefs, computed, Ref, ComputedRef, nextTick, inject } from 'vue';
