<template>
	...
	>
		<span class="title" v-if="list[0].path.indexOf('/workIndex') > -1">{{ $t(flowableStore.itemName) }}</span>
		<span class="title" v-else-if="list[0].path == '/index'">{{ $t(list[1].meta.title) }}</span>
		<span class="title" v-else-if="list[0].path == '/cplane'">{{ $t(list[0].meta.title) }}</span>
		<span class="title" v-else>{{ $t(list[0].meta.title) }}</span>
...
<script lang="ts">
	...
	import LeftVue from '@/layouts/Y9-default/Left.vue';
