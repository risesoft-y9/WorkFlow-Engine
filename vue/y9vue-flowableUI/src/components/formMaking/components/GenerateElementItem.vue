<template>
  <div :class="{'print-read-label': printRead}" style="width: 100%;">
    ...
    <template v-if="widget.type == 'custom'">  
      ...
      <!-- 自定义附件列表组件 -->
       <!-- 自定义正文组件 -->
       <div v-else-if="widget.el == 'custom-word'" :style="{width: isTable ? '100%' : widget.options.width}">
        <component
          :is="widget.el"
          :ref="'fm-'+widget.model"
          v-model="dataModel"
    ...
    <template v-if="widget.type == 'table'">
      <!-- 子表 -->
      <template v-if="widget.model.indexOf('childTable@') > -1">
        <fm-form-table
          :ref="'fm-'+widget.model"
          ...
          :tableId="widget.childTableInfo.split('@')[1]"
          :tableName="widget.childTableInfo.split('@')[0]"
          :widget="widget"
        >
          <template v-for="blank in blanks" v-slot:[blank.name]="scope">
            <slot :model="scope.model" :name="blank.name"></slot>
          </template>
        </fm-form-table>
      </template>
      <template v-else>
