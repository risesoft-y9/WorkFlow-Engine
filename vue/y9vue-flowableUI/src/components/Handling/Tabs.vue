...
<template>
    <div class="tabs-style">
        <div style="display: flex">
            <div v-for="(item, index) in tabsList" :key="index" class="item-style" @click="handlerClick(item, index)">
                <div :class="{ 'is-active': item.name === activeIndex }" class="name">
                    <div style="writing-mode: vertical-lr"
                        >{{
                            fontSizeObj.buttonSize == 'large' && item.label.length > 7
                                ? `${$t(item.label?.slice(0, 7))}...`
                                : $t(item.label)
                        }}
                    </div>
                    <span v-if="item.number" style="height: 20px">{{ $t(item.number) }}</span>
                </div>
                <div :class="{ 'triangle-active': item.name === activeIndex }" class="triangles"></div>
            </div>
        </div>
    </div>
</template>
<script lang="ts" setup>
    ...
    let { tabsList, activeIndex } = toRefs(data);
