<template>
      <div class="div1">
        ...
        <el-button type="primary" size="small"
          :style="{ fontSize: fontSizeObj.smallFontSize }" style="margin-bottom: 10px;" @click="setRemind">
       ...
       <el-row :gutter="20">
         <el-col :span="16" >
            <y9Table v-model:selectedVal="remindData" ref="reminderTableRef" :config="reminderTableConfig" @select="handleSelectionChange" @select-all="handleSelectionChange"></y9Table>
         </el-col>
         <el-col :span="8" class="right-checkbox">
            <y9Table :config="nodeTableConfig">
              <template #optButton="{row,column,index}">
                  <el-checkbox border size="small" style="margin-right:5px;" v-model="boxArriveGroup" :label="row.taskDefKey+':'+row.taskDefName" @change="handleNodeArrived">{{ $t('到达') }}</el-checkbox>
                  <el-checkbox border size="small" v-model="boxDoneGroup"  :label="row.taskDefKey+':'+row.taskDefName" @change="handleNodeDone">{{ $t('完成') }}</el-checkbox>
              </template>
          </y9Table>
         </el-col>
       </el-row>
...
<script lang="ts" setup>
  ...
  import {remindTaskList,getBpmList,saveRemindInstance} from '@/api/flowableUI/reminder'
...
const data = reactive({
  ...
  function handleAllTaskDone(val){//取消全选
    if(!val){
       reminderTableRef.value.elTableRef.clearSelection();
    }
  }

  function handleAllNodeArrived(){//取消节点选中
    boxArriveGroup.value = [];
  }

  function handleNodeArrived(){//列表中有选中项时选中全选按钮
  ...
  function handleNodeDone(){//列表中有选中项时选中全选按钮
  ...
  function clear(){//清空数据
    ...
    function setRemind(){ //保存
      ...
      let task_Ids = selectedRemindData.value.map(item=>{
          return item.taskId;
      }).join(',');
      let arriveTaskKey = boxArriveGroup.value.map(item=> item).join(',');
      let completeTaskKey = boxDoneGroup.value.map(item=> item).join(',');
      saveRemindInstance(props.processInstanceId,task_Ids,process.value,arriveTaskKey,completeTaskKey).then(res=> {
        if (res.success){
