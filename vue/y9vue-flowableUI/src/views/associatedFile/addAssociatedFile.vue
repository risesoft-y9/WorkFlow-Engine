<template>
    <y9Table :config="fileTableConfig" :filterConfig="filterConfig" @select="handleSelectionChange" @select-all="handleSelectionChange"  @on-curr-page-change="onCurrPageChange"
            @on-page-size-change="onPageSizeChange">
            <template #fileOpt>
              <el-input v-model="title" class="addAssociatedFileInput"  :placeholder="$t('请输入标题文号')" clearable></el-input>
              <el-button type="primary" @click="reloadTable()" style="margin-left:10px;" :size="fontSizeObj.buttonSize"
                :style="{ fontSize: fontSizeObj.baseFontSize }"
              ><i class="ri-search-line"></i>{{$t('搜索')}}</el-button>
              <el-button type="primary" @click="saveAssociated" style="margin-left:10px;" :size="fontSizeObj.buttonSize"
