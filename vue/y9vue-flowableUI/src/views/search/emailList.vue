<template>
   <y9Table ref="filterRef" :filterConfig="filterConfig" :config="tableConfig" @on-curr-page-change="onCurrPageChange" @on-page-size-change="onPageSizeChange">
      <template #search>
        <el-button type="primary" @click="reloadTable" :size="fontSizeObj.buttonSize"
      ...
      <template  #title="{row,column,index}">
          <el-link v-if="!row.read" style="color: blue;" :underline="false" @click="openEmail(row)">{{row.subject}}</el-link>
...
import {getEmailList} from "@/api/flowableUI/search";
