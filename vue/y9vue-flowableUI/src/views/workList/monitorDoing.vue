...
<script lang="ts" setup>
    ...
    import { getmonitorDoingList, removeProcess } from '@/api/flowableUI/monitor';
...
import { computed } from 'vue';
    ...
    const data = reactive({
        ...
        tableConfig: {
            ...
            columns: [
                ...
                { title: computed(() => t('标题')), key: 'documentTitle', slot: 'documentTitle', align: 'left', minWidth: '200' },
                { title: computed(() => t('发起人')), key: 'creatUserName', width: '180' },
                { title: computed(() => t('办理环节')), key: 'taskName', width: '120' },
                { title: computed(() => t('办理人')), key: 'taskAssignee', width: '180' },
                { title: computed(() => t('开始时间')), key: 'taskCreateTime', width: '150' },
    ...
    async function reloadTable() {
        ...
        let res = await getmonitorDoingList(itemId.value, currFilters.value.name, page, rows);
    ...
    function openDoc(row) {
        let query = {
            ...
            itembox: 'monitorDoing',
            taskId: row.taskId,
            processInstanceId: row.processInstanceId,
            listType: 'monitorDoing'
        };
        flowableStore.$patch({
            //设置打开当前页
            currentPage: tableConfig.value.pageConfig.currentPage.toString()
