      ...
      <div class="block"
        ...
        <el-timeline>
          <template v-for="task,index in taskList">
            <el-timeline-item v-if="index == 0" :timestamp="task.taskName" placement="top" v-bind:key="index">
              <el-card>
                <el-select v-if="task.add" v-model="taskKey" :placeholder="$t('添加节点')" :size="fontSizeObj.buttonSize"
                 style="margin-bottom: 10px;" @change="addTask(index)">
                  <el-option
            ...
            <el-timeline-item v-else :timestamp="task.taskName" placement="top" :[key]="index">
              <el-card>
                <div>
                  <el-select v-if="task.add && task.type != 'endEvent'" v-model="taskKey" :placeholder="$t('添加节点')" :size="fontSizeObj.buttonSize"
                   @change="addTask(index)" style="margin-bottom: 10px;">
                    <el-option
                      v-for="item in taskNodes"
                      :[key]="item.taskDefKey"
                      :label="item.taskDefName"
                  ...
                  <i v-if="task.delete" :title="$t('删除任务')" @click="delTask(index)" class="el-icon-circle-close" :style="{fontSize: fontSizeObj.mediumFontSize,color: '#888',float: 'right'}"></i>
                </div>
                <el-divider v-if="task.type != 'endEvent'" content-position="left">{{ $t('办理人') }}<i class="ri-user-settings-line" :title="$t('办理人设置')" @click="userSetting(task.taskKey,index,task.orgList)" :style="{fontSize: fontSizeObj.mediumFontSize}"></i></el-divider>
        ...
        <div style="text-align: right;margin: 15px;">
          <span slot="footer" class="dialog-footer">
            ...
            :style="{ fontSize: fontSizeObj.baseFontSize }" @click="saveProcess">{{ $t('确定') }}</el-button>
            <el-button :size="fontSizeObj.buttonSize"
              :style="{ fontSize: fontSizeObj.baseFontSize }" @click="cancel">{{ $t('取消') }}</el-button>
          </span>
        </div>
      </div>
      <el-drawer
...
import {getTargetNodes,getContainEndEvent4UserTask,saveCustomProcess} from "@/api/flowableUI/buttonOpt";
...
function cancel(){
  ...
  function delTask(index){//删除任务节点
    ElMessageBox.confirm(t("确定删除该任务?"), t("提示"), {confirmButtonText: t("确定"),cancelButtonText: t("取消"),type: "info", appendTo: '.block'}).then(() => {
  ...
  function userSetting(taskKey,index,orgList){//办理人选择
      drawer.value = true;
  ...
  function setUser(orgList){//设置办理人
      drawer.value = false;
  ...
  function handleClose(id,index){//删除办理人
  ...
  function saveProcess(){//保存流程定制信息并发送
      if(taskList.value[taskList.value.length - 1].type != "endEvent"){
        getContainEndEvent4UserTask(processDefinitionId.value).then(res => {
          if(res.success){
            ...
            ElMessage({ type: "error", message: t("流程未配置结束节点，") + str,offset:65, appendTo: '.block'});
          }
        });
        return;
      }
      ...
      if(taskName != ""){
        ElMessage({ type: "error", message: t("任务节点")+"【"+taskName+"】"+t("未配置办理人"),offset:65, appendTo: '.block'});
        return;
      }
      ElMessageBox.confirm(t("确定生成该流程定制并发送?"), t("提示"), {confirmButtonText: t("确定"),cancelButtonText: t("取消"),type: "warning", appendTo: '.block'}).then(() => {
        let jsonData = JSON.stringify(taskList.value).toString();
        loading.value = true;
        saveCustomProcess(basicData.value.itemId,basicData.value.processSerialNumber,basicData.value.processDefinitionKey,jsonData).then(res => {
