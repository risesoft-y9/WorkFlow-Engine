<template>
  <el-container class="personTree" style="height:480px;">
    ...
    <el-main  width="45%" style="height:auto;overflow: hidden; border-top: 0px solid #ccc;border-left: 0;border-right: 0;border-bottom: 0;">
      <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" ref="kzMenu"  @select="handleSelect">
        <el-menu-item v-if="existPerson" index="3">{{ $t('人员') }}</el-menu-item>
        <el-menu-item v-if="existPosition" index="6">{{ $t('岗位') }}</el-menu-item>
...
<script lang="ts" setup>
  ...
  let {
    ...
    selectField,
	 ...
	 const onTreeLazyLoad = async (node, resolve: () => void) => {
		if(principalType.value == 0){//刚初始化时不加载
			return;
		}
		 if (node.$level === 0) {
			 ...
			 //1.获取数据
			 let data = [];
			 const res = await findAllPermUser(props.basicData.itemId,props.basicData.processDefinitionId,
      						userChoiseData.value.routeToTask,principalType.value,props.basicData.processInstanceId);//请求一级节点接口
			 data = res.data || res;
			 
			if(userChoiseData.value.multiInstance == "common" || "yewufuzerenshenhe" == userChoiseData.value.routeToTask || props.reposition == "reposition"){
				selectField.value[0].value = ['Position'];
			}
			if(props.fromType == "加减签"){//加减签选择，只能选岗位
				selectField.value[0].value = ['Position'];
			}
			 ...
			 if(node.children && node.children.length > 0){//如果有传入的数据就取传入的数据
				 ...
				 //请求接口
				 const res = await findAllPermUser(props.basicData.itemId,props.basicData.processDefinitionId,userChoiseData.value.routeToTask,principalType.value,
                                        props.basicData.processInstanceId,params.parentId);
				 data = res.data || res;
			 }
	 ...
	 async function search(){
		  if(searchName.value){//有值就请求搜索api
				 ...
				 let params = {
					...
					processDefinitionId: props.basicData.processDefinitionId,
					taskDefKey: userChoiseData.value.routeToTask,
				 ...
				 //请求搜索接口
				 const res = await findPermUserByName(params);
  ...
  function findPermUser(choiseData){
    searchName.value = "";
    userChoiseData.value = choiseData;

    existPerson.value = false;
    existDepartment.value = false;
    existCustomGroup.value = false;
    existPosition.value = false;
    if(userChoiseData.value.existCustomGroup){
      principalType.value = 7;
      existCustomGroup.value = userChoiseData.value.existCustomGroup;
    }
    if(userChoiseData.value.existDepartment){
      principalType.value = 2;
      existDepartment.value = userChoiseData.value.existDepartment;
    }
    if(userChoiseData.value.existPerson){
      principalType.value = 3;
      existPerson.value = userChoiseData.value.existPerson;
    }
    if(userChoiseData.value.existPosition){
      principalType.value = 6;
      existPosition.value = userChoiseData.value.existPosition;
    }
    activeIndex.value = principalType.value.toString();
    onRefreshTree();
  }
