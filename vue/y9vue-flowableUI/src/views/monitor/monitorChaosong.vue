...
import { computed } from 'vue';
    ...
    const data = reactive({
        ...
        tableConfig: {
            ...
            columns: [
                ...
                { title: computed(() => t('状态')), key: 'banjie', width: '55', slot: 'itembox' },
        ...
        filterConfig: {
            ...
            itemList: [
                ...
                },
                {
                    type: 'input',
                    key: 'senderName',
                    label: computed(() => t('发送人')),
                    props: {
                        placeholder: computed(() => t('请输入发送人'))
                    },
                ...
                },
                {
                    type: 'input',
                    key: 'userName',
                    label: computed(() => t('收件人')),
                    props: {
                        placeholder: computed(() => t('请输入收件人'))
                    },
                    span: settingStore.device === 'mobile' ? 6 : 3
                },
                {
    ...
    async function reloadTable() {
        ...
        let res = await monitorChaosongList(
            currFilters.value.name,
            currFilters.value.itemId,
            currFilters.value.senderName,
    ...
    function handleDelete() {
        if (multipleSelection.value.length === 0) {
            ElMessage({ type: 'error', message: t('请选择要删除的抄送件'), offset: 65, appendTo: '.y9-table-div' });
            return;
        } else {
            let ids = [];
            ...
            ElMessageBox.confirm(t(`确定删除抄送件?`), t('提示'), {
                ...
                .then(async () => {
                    let res = await deleteList(ids.join(','));
                    ElNotification({
