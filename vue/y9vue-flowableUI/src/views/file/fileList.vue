...
<template>
    ...
    >
        <el-aside style="width: 100%; height: auto; overflow: auto; padding: 1% 0% 2% 0%" class="formaside">
            <y9Card :showHeader="false" style="width: 80%; height: calc(100% - 20px); margin: auto">
                <div v-if="basicData.itembox == 'add' || basicData.itembox == 'draft' || basicData.itembox == 'todo'" class="margin-bottom-20 fileList">
                    <el-button type="primary" @click="addFiles" plain :size="fontSizeObj.buttonSize"
                        :style="{ fontSize: fontSizeObj.baseFontSize }"><i class="ri-file-add-line" ></i>{{ $t('添加') }}</el-button>
                    <a v-if="downloadZipShow" :href="downloadZipUrl+'&processSerialNumber='+basicData.processSerialNumber" style="margin: 0 12px;">
                        <el-button type="primary" plain :size="fontSizeObj.buttonSize"
                        :style="{ fontSize: fontSizeObj.baseFontSize }"><i class="ri-folder-download-line" ></i>{{ $t('zip下载') }}</el-button>
                    </a>
                    <el-button type="primary" @click="delFile" :size="fontSizeObj.buttonSize"
                    :style="{ fontSize: fontSizeObj.baseFontSize }"><i class="ri-delete-bin-line"></i>{{ $t('删除') }}</el-button>
                </div>
                <div v-if="basicData.itembox != 'add' && basicData.itembox != 'draft' && basicData.itembox != 'todo' && downloadZipShow" class="margin-bottom-20 fileList">
                    <a :href="downloadZipUrl+'&processSerialNumber='+basicData.processSerialNumber" style="margin: 0 12px;">
                        <el-button type="primary" plain :size="fontSizeObj.buttonSize"
                        :style="{ fontSize: fontSizeObj.baseFontSize }"><i class="ri-folder-download-line" ></i>{{ $t('zip下载') }}</el-button>
                    </a>
                </div>
                ...
                >
                    ...
                    <template #opt="{ row, column, index }">
                        <a style="text-decoration:none;" :href="downloadUrl+'&id='+row.id"><i class="ri-download-2-line" :title="$t('点击下载')" style="font-size: 18px"></i></a>
                    </template>
                </y9Table>
...
<script lang="ts" setup>
    import { ref, defineProps, onMounted, toRefs, reactive, inject } from 'vue';
    import { ElMessage, ElMessageBox, ElLoading } from 'element-plus';
    import { getAttachmentList, delAttachment, saveAttachment } from '@/api/flowableUI/attachment';
...
import { computed } from 'vue';
    ...
    const data = reactive({
        ...
        fileTableConfig: {
            columns: [
                ...
                { title: computed(() => t('名称')), type: 'name', slot: 'name', width: 'auto', align: 'left',minWidth:'200' },
    ...
    onMounted(() => {
        ...
        downloadUrl.value = import.meta.env.VUE_APP_HOST + import.meta.env.VUE_APP_NAME +'/vue/attachment/attachmentDownload?access_token=' + y9_storage.getObjectItem(settings.siteTokenKey, 'access_token')
        downloadZipUrl.value = import.meta.env.VUE_APP_HOST + import.meta.env.VUE_APP_NAME +'/vue/attachment/packDownload?access_token=' + y9_storage.getObjectItem(settings.siteTokenKey, 'access_token')
        if (
    ...
    function reloadTable() {    
        ...
        getAttachmentList(props.basicData.processSerialNumber, page, rows).then((res) => {
            ...
            for (let i in fileList) {
                ...
                if(png.indexOf(type) > -1){//图片使用外网地址，使用前端配置
                    fileList[i].downloadUrl = import.meta.env.VUE_APP_HOST + "itemAdmin" + fileList[i].downloadUrl.split('/itemAdmin')[1];
                    fileList[i].jodconverterURL = import.meta.env.VUE_APP_JODCONVERTERURL + fileList[i].downloadUrl;
                }
            }
    ...
    function addFiles() {
        ...
        upload.value.url = import.meta.env.VUE_APP_HOST + import.meta.env.VUE_APP_NAME +'/vue/attachment/upload?access_token=' +y9_storage.getObjectItem(settings.siteTokenKey, 'access_token');
        ...
        if (multipleSelection.value.length === 0) {
            ...
            for (let item of multipleSelection.value) {
                ...
                if (y9UserInfo.value.personId != item.personId) {
                    ElMessage({ type: 'error', message: t('只能删除自己上传的文件'), offset: 65, appendTo: '.filecontainer' });
                    return;
                }
            }
