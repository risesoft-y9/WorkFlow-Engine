<template>
    <div style="width: 690px; height: 97.7%; margin: 15px auto" class="from-file">
        <div style="padding-bottom: 15px; vertical-align: middle">
            <div class="att-files">{{ $t('附件')}}</div>
            <div class="addFile-bnt" v-if="optShow">
                <i class="ri-file-add-line" @click="addFiles" :title="$t('添加附件')"></i>
            </div>
            <div class="downloaFiles-bnt" v-if="downloadZipShow">
                <a style="text-decoration:none;" :href="downloadZipUrl+'&processSerialNumber='+basicData.processSerialNumber">
                    <i class="ri-folder-download-line" @click="downloaFiles" :title="$t('下载附件zip')"></i>
                </a>
            </div>
        </div>
        ...
        <y9Table :config="fileTableConfig">
            ...
            <template #opt="{ row, column, index }">
                <a style="text-decoration:none;" :href="downloadUrl+'&id='+row.id"><i class="ri-download-2-line" :title="$t('点击下载')" :style="{fontSize: fontSizeObj.mediumFontSize}"></i></a>
                <i
        ...
        <y9Dialog v-model:config="dialogConfig" class="addfileDialog">
            ...
            <div style="height: auto;width: 100%;min-height: 200px;max-height: 500px;">
                <y9Upload v-if="upload.url != ''" :upload="upload" @getFileData='getFileData'>
                </y9Upload>
            </div>
        </y9Dialog>
    </div>
