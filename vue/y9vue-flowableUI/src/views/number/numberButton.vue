 ...
 defineExpose({
  ...
  async function updateNumber(formData){
    // if(numberData.value.itembox == 'todo'){
      //获取编号按钮权限
      await findByCustom(numberData.value.itemId, numberData.value.processDefinitionId, numberData.value.taskDefKey, props.numberCustom).then(async res => {
        if(res.success){
  ...
  async function getY9Number(type){
    await getNumber(numberData.value.itemId, props.numberCustom, characterValue.value, year.value).then(async res => {
      if(res.success){
        let nowDate = new Date();
        ...
        if(type == '1'){
          ElMessage({ type: "success", message: t("编号已变更"),offset:65, appendTo: '.number-button'});
        }
        await saveNumber();
      }
    })
  ...
  async function saveNumber(){//保存编号
    let msg = false;
    let res = await checkNumber(numberData.value.itemId, props.numberCustom,characterValue.value,year.value,number.value,numberData.value.processSerialNumber);
    if(res.success){
