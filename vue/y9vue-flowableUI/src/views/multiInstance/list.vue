<template>
    <div
      ...
      element-loading-background="rgba(0, 0, 0, 0.8)" class="control">
      <div class="margin-bottom-20" v-if="type=='parallel'">
        <el-button type="primary" @click="addInstance" :size="fontSizeObj.buttonSize"
        :style="{ fontSize: fontSizeObj.baseFontSize }"><i class="ri-add-line"></i>{{ $t('加签') }}</el-button>
      </div>
      <y9Table :config="tableConfig">
          <template #optButton="{row,column,index}">
            <el-button size="small"
              :style="{ fontSize: fontSizeObj.smallFontSize }" v-if="type == 'sequential' && (row.status =='正在办理' || row.status == '未开始')" @click="addInstance(row)" type="primary" ><i class="ri-add-line"></i>{{ $t('加签') }}</el-button>
            <el-button size="small"
              :style="{ fontSize: fontSizeObj.smallFontSize }" v-if="type == 'parallel' || (type == 'sequential' && row.status =='未开始')" @click="delExecution(row)" type="primary" ><i class="ri-subtract-line"></i>{{ $t('减签') }}</el-button>
            <el-button size="small" :class="{'margin-button': fontSizeObj.buttonSize == 'large' }"
              :style="{ fontSize: fontSizeObj.smallFontSize }" v-if="type == 'parallel'" @click="sponsor(row.taskId)" type="primary" ><i class="ri-user-settings-line"></i>{{ $t('设为主办') }}</el-button>
          </template>
      </y9Table>
    </div>
    <y9Dialog v-model:config="dialogConfig">
      <userChoise ref="userChoiseRef" @reloadTable="multiInstanceList" :dialogConfig="dialogConfig" :basicData="basicData" :fromType="$t(fromType)" :routeToTask="taskDefinitionKey" :instanceData="instanceData"/>
    </y9Dialog>
</template>
...
import {getAddOrDeleteMultiInstance,setSponsor,removeExecution4Sequential,removeExecution} from "@/api/flowableUI/multiInstance";
...
const data = reactive({
 ...
 function multiInstanceList(){
      ...
      getAddOrDeleteMultiInstance(props.basicData.processInstanceId).then(res => {
        ...
        if(type.value == 'sequential'){
          tableConfig.value.columns.splice(2,0,{ title: computed(() => t("任务状态")), key: "status", width: '150', });
          for(let item of res.data.rows){
            if(item.status == '未开始'){//串行加签后存在未开始人员，如果有发送按钮，提交按钮，办结按钮等，要提醒重新打开办件处理
              emits("refreshButton");
            }
          }
          if(res.data.rows[res.data.rows.length - 1].status == '正在办理'){//串行减签只剩自己最后一人，送下一人按钮需要取消
            emits("refreshButton");
          }
        }else if(type.value == 'parallel'){
          tableConfig.value.columns.splice(2,0,{ title: computed(() => t("是否主办")), key: "isZhuBan", width: '150', });
          for(let item of res.data.rows){
          ...
          if(res.data.rows.length == 1){//并行减签只剩自己最后一人，办理完成按钮需要取消
            emits("refreshButton");
          }
        }
  ...
  function addInstance(row){
      ...
      if(type.value == 'sequential'){
        ...
        ElMessageBox.confirm(`${t('确定在')}【${selectUserName}】${t('之后添加办理人吗')}？`, t("提示"), {confirmButtonText: t("确定"),cancelButtonText: t("取消"),type: "info", appendTo: '.control'}).then(() => {
 ...
 function delExecution(row){
    ...
    var selectUserName = row.assigneeName;
    ElMessageBox.confirm(`${t('真的删除')}【${selectUserName}】${t('办理人员吗？')}`, t("提示"), {confirmButtonText: t("确定"),cancelButtonText: t("取消"),type: "info", appendTo: '.control'}).then(() => {
      if(type.value == 'sequential'){
