<template>
  <el-container :style="style"
    ...
    <div class="document" :class="{ 'document-shrink': !processFlag}">
      <div class="buttonDiv">
        <FloatButton :list="operationBtnList"/>
      </div>
      <div class="tab-nav">
        <Tabs class="nav" v-model:activeName="activeName" :list="dataList"  @tab-click="tabClick"> </Tabs>
      </div>
    ...
    <div class="process-content" :style="processStyle" v-if="processFlag">
        <el-card style="height:100%;box-shadow:0px 0px 0px 0px #333;">
          ...
          <div style="height: 94%;overflow-y: auto;overflow-x: hidden;display: flex;background-color: white;
          justify-content: center;align-items: self-start;">
          <!-- 没数据 显示暂无数据 -->
...
import { ref, defineProps, onMounted, onBeforeMount,reactive, inject} from 'vue';
...
const data = reactive({
  ...
  function updateLeftListCount(){
    ...
    async function getChaoSongData(){//获取抄送数据
      loading.value = true;
    ...
    dataList.value.push({label: formList.value[0].formName, name: 'y9form'});
      ...
      for(let item of menuMap.value){
        if(item.menuKey.indexOf('follow') > -1 && follow.value){ //关注按钮：取消关注
          operationBtnList.value.push({name:'关注',icon:'ri-star-fill',
            onClick: () => {
              delFollow()
            }
          });
        }else if(item.menuKey.indexOf('follow') > -1 && !follow.value){//关注按钮：点击关注
          operationBtnList.value.push({name:'关注',icon:'ri-star-line',
            onClick: () => {
              saveFollow()
            }
          });
          ...
          operationBtnList.value.push({name:item.menuName,icon:iconName,
            onClick: () => {
