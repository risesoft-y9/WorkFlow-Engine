...
<template>
  <el-container style="height: 100%; width: 100%;display: block;">
    <div style="width: 100%;text-align:right;margin: 15px 0px;">
      ...
      <el-button ref="buttonRef" type="primary" :size="fontSizeObj.buttonSize" @click="addShuiyin" style="margin-left: 10px;"
        :style="{ fontSize: fontSizeObj.baseFontSize }">
        <i class="ri-home-6-line"></i><span style="margin-left: 5px;">{{ $t(shuiyinStr) }}</span>
      </el-button>
      <el-button ref="buttonRef" type="primary" style="margin-right: 2%;" v-print="'#printTest'" :size="fontSizeObj.buttonSize"
        :style="{ fontSize: fontSizeObj.baseFontSize }">
        <i class="ri-printer-line"></i><span style="margin-left: 5px;">{{ $t('打印') }}</span>
      </el-button>
    </div>
    <div ref="printTestRef" class="printTest" style="background-color: #fff;position: absolute;left: 50%;transform: translate(-50%, -50%);" id="printTest">
      <fm-generate-form
        :data="formJson"
        :edit="edit"
        ref="generateFormRef"
...
import { ref, defineProps, onMounted,nextTick,reactive, inject} from 'vue';
import {getFormJson,getFormData,getChildTableData} from "@/api/flowableUI/form";
...
onMounted(()=>{
  ...
  if(typeof watermarkValue.value == 'string') {
    cans.value.fillText(watermarkValue.value ? watermarkValue.value : '', can.value.width / 4, can.value.height / 2);
  }else if(typeof watermarkValue.value == 'object') {
    let strValue = `${watermarkValue.value?.name ? watermarkValue.value?.name : ''}${watermarkValue.value?.deptName ?  '-' + watermarkValue.value?.deptName : ''}${watermarkValue.value?.userIP ? '-' + watermarkValue.value?.userIP : ''}`;
...
async function initChildTable(){//加载子表数据
