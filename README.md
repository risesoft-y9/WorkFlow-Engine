...
| 序号 | 特点     | 特点描述                                                     |
| ---- | -------- | ------------------------------------------------------------ |
| 1    | 串行办理 | 一个liu程节点，多人按照顺序依次收到待办进行办理，全部办理完成后，自动进入下一个liu程节点 |
| 2    | 并行办理 | 一个liu程节点，多人同时收到待办进行办理，全部办理完成后，自动进入下一个liu程节点 |
| 3    | 或行办理 | 一个liu程节点，无论串行或并行办理，任意一个人办理完成后，自动进入下一个liu程节点（需避免使用排他网关） |
| 4    | 票行办理 | 一个liu程节点，无论串行或并行办理，多个人分配不同的权重进行投票，投票权重大于X%(例如60%），自动进入下一个liu程节点（需避免使用排他网关） |
| 5    | 单人签收 | 一个liu程节点，支持多人同时接到办理任务，一个人抢占签收进行办理（签收后，其他人无办理任务），可拒签，可撤销签收 |
| 6    | 主协办   | 一个并行办理的liu程节点，支持定义主办和协办，只有主办可以办理完成后操作进入一个liu程节点 |
| 7    | 子liu程  | 主liu程设置子liu程，支持主、子liu程独立办理                  |
| 8    | 自动执行 | 支持在liu程路径和liu程节点自动触发或者跳过，并进入下一个liu程节点 |
| 9    | 加减签   | 支持在本liu程节点增加或者减少办理的岗位                      |
| 10   | 抄送     | 任何liu程状态下，所有参与人都可以将本liu程实例通知至其他岗位，允许重复抄送 |
| 11   | 重定位   | 支持将当前liu程实例跳转到任意节点                            |
| 12   | 委托     | 将本岗位的所有事项和liu程实例交由另一个岗位进行办理，支持设置委托的日期范围 |
| 13   | 催办     | 支持在在办列表中对于办理岗位进行催促通知                     |
| 14   | 退回     | 支持将本liu程实例退回给申请人（起草人),支持退回给liu程节点的发送人 |
| 15   | 收回     | 支持发送人在当前办理人未liu转前，撤回并重新办理              |
| 16   | 特殊办结 | 无论在任何liu程节点，支持直接对liu程实例进行办结             |
| 17   | 过期任务 | 支持liu程实例在每个liu程节点配置过期时间，过期后进行状态改变 |
| 18   | 恢复待办 | 支持将已办结的liu程实例重新进行liu转                         |

## 引擎国产化功能

| 序号 | 功能      | 功能描述                                                     |
| ---- | --------- | ------------------------------------------------------------ |
| 1    | 事项配置  | 针对所有工作liu引擎中的配置项进行统一的绑定、管理操作        |
| 2    | 表单配置  | 支持表单的动作设置、数据源、字段绑定、字段校验、样式加载等诸多的灵活配置项 |
| 3    | 统一待办  | 汇总所有的待办消息于统一界面进行通知和办理                   |
| 4    | 接口配置  | 支持第三方系统的接口配置、测试和调用，便于在liu程路径和liu程节点中进行接入 |
| 5    | 链接配置  | 支持第三方系统的功能模块地址的绑定和配置                     |
| 6    | 关联文件  | 支持手动将参与办理的实例与当前实例进行关联，关联后可以联动打开 |
| 7    | 消息提醒  | 支持对在办件的liu程节点的意见批复和办理情况进行消息通知配置  |
| 8    | 实例迁移  | 支持将低版本的liu程的实例无缝迁移至高版本中                  |
| 9    | liu程监控 | 支持对liu程实例的删除、挂起、激活、历程图、变量进行实时查看和操作 |
| 10   | 综合检索  | 针对当前登录人的岗位下所有事项中的所有状态的liu程实例检索    |
| 11   | 正文模板  | 支持外部导入的liu式文件模板，便于正文起草的模板使用（需要正文插件） |
| 12   | 套红模板  | 支持外部导入的liu式红头文件模板，便于正文起草后进行红头模板的添加（需要正文插件） |
| 13   | 沟通交liu | 在liu程实例的全过程中，支持所有办理和抄送相关岗位的人员进行留言沟通 |
| 14   | 打印模板  | 支持表单信息和word模板的配置，便于liu程liu转过程中的预览和打印 |
| 15   | 日历配置  | 针对法定节假日、国家调休计算工作日和自然日，便于liu程的日期规则计算 |
| 16   | 意见配置  | 在事项的不同的liu程节点中，支持配置不同类型的意见框，从而明确不同岗位的意见分栏 |
| 17   | 动态视图  | 支持灵活配置草稿箱、待办列表、在办列表、办结列表的展示方式   |
| 18   | 编号配置  | 在事项的不同的liu程节点中，支持授权岗位对字段进行编号的定义  |
