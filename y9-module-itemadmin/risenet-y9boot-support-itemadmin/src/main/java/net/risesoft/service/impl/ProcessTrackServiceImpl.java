...
public class ProcessTrackServiceImpl implements ProcessTrackService {
    ...
    private List<net.risesoft.model.processadmin.HistoricActivityInstanceModel>
        getActivityListByProcessInstanceId(String processInstanceId) {

        List<net.risesoft.model.processadmin.HistoricActivityInstanceModel> list =
            this.historicActivityApi.getByProcessInstanceIdAndYear(processInstanceId, "").getData();

        if (list == null || list.isEmpty()) {
            // 尝试从办结信息中获取年份
            String year = getYearFromOfficeDoneInfo(processInstanceId);
            if (StringUtils.isNotBlank(year)) {
                list = this.historicActivityApi.getByProcessInstanceIdAndYear(processInstanceId, year).getData();
            }
        }
     ...
     * 获取已分配人员列表
     */
    private List<Person> getAssignedPersonList(HistoricTaskInstanceModel hai, String taskId, String assignee) {
        List<Person> personList;

        if (null != hai.getClaimTime()) {
            personList = getClaimedPersonList(hai, taskId, assignee);
        } else {
     ...
     * 获取已签收人员列表
     */
    private List<Person> getClaimedPersonList(HistoricTaskInstanceModel hai, String taskId, String assignee) {
        List<Person> personList = new ArrayList<>();
        try {
            List<IdentityLinkModel> iList = this.historicIdentityApi.getIdentityLinksForTask(taskId).getData();
     ...
     * 获取历史任务实例结果
     */
    private HistoricTaskInstanceResult getHistoricTaskInstances(String processInstanceId) {
        List<HistoricTaskInstanceModel> results =
            this.historictaskApi.getByProcessInstanceId(processInstanceId, "").getData();
        String year = "";

        if (results == null || results.isEmpty()) {
            year = getProcessYear(processInstanceId);
            results = this.historictaskApi.getByProcessInstanceId(processInstanceId, year).getData();
        }
    ...
    private HistoryProcessModel getHistoryProcessModel(HistoricTaskInstanceModel hai, int tabIndex) {
        ...
        try {
            ...
            // 设置待办状态
            setNewToDoStatus(model, taskId, hai);
            // 设置办理人信息
            setAssignee(model, hai, taskId);
            // 设置时间信息
    ...
    private void handleDeleteMIExecutionDescription(HistoryProcessModel model, HistoricTaskInstanceModel hai,
        String year) {
        // 获取任务发送者信息
        String taskSender = getTaskSender(hai.getId(), year);
        if (StringUtils.isNotBlank(taskSender)) {
            model.setDescription("该任务由" + taskSender + "删除");
        }

        // 获取回退原因并更新描述
        String rollBackReason = getRollBackReason(hai.getId(), year);
     ...
     * 处理串行办理情况
     */
    private void handleSequentialProcessing(List<HistoryProcessModel> items, String processInstanceId) {
        ...
        try {
            ...
            processSequentialUsers(items, processInstanceId);
        } catch (Exception e) {
    ...
    public List<HistoryProcessModel> listByProcessInstanceId(String processInstanceId) {
        ...
        try {
            // 获取历史任务实例列表
            HistoricTaskInstanceResult result = getHistoricTaskInstances(processInstanceId);
            ...
            for (HistoricTaskInstanceModel hai : results) {
                ...
                HistoryProcessModel model = processHistoricTaskInstance(hai, year);
                items.add(model);
                // 处理流程跟踪信息
            ...
            // 处理串行办理情况
            handleSequentialProcessing(items, processInstanceId);
    ...
    public List<HistoryProcessModel> listByProcessInstanceId4Simple(String processInstanceId) {
        ...
        try {
            ...
            for (HistoricTaskInstanceModel hai : results) {
                ...
                HistoryProcessModel history = processSimpleHistoricTaskInstance(hai, year);
                items.add(history);
    ...
    public List<HistoryProcessModel> listByProcessInstanceIdWithActionName(String processInstanceId) {
        ...
        try {
            ...
            // 获取子流程节点列表
            List<TargetModel> subNodeList = getSubProcessNodeList(results.get(0).getId(), year);
     ...
     * 处理历史任务实例
     */
    private HistoryProcessModel processHistoricTaskInstance(HistoricTaskInstanceModel hai, String year) {
        ...
        try {
            ...
            // 设置办理人信息
            setAssigneeInfo(model, hai, year);

            // 设置待办状态
            setNewToDoStatus(model, hai);

            // 设置结束标识
            model.setEndFlag(StringUtils.isBlank(hai.getTenantId()) ? "" : hai.getTenantId());

            // 设置描述信息
            setDescriptionInfo(model, hai, year);
     ...
     * 处理简单历史任务实例
     */
    private HistoryProcessModel processSimpleHistoricTaskInstance(HistoricTaskInstanceModel hai, String year) {
        HistoryProcessModel history = createSimpleBaseModel(hai);

        try {
            // 设置办理人信息
            setSimpleAssigneeInfo(history, hai, year);
     ...
     * 设置已分配用户信息
     */
    private void setAssignedInfo(HistoryProcessModel model, HistoricTaskInstanceModel hai, String taskId,
        String assignee) {
        OrgUnit employee = this.orgUnitApi.getPersonOrPosition(Y9LoginUserHolder.getTenantId(), assignee).getData();
        ...
        // 设置人员列表
        List<Person> personList = getAssignedPersonList(hai, taskId, assignee);
     ...
     * 设置已分配用户信息
     */
    private void setAssignedUserInfo(HistoryProcessModel model, HistoricTaskInstanceModel hai, String year,
        String assignee) {
        try {
            OrgUnit employee = this.orgUnitApi.getPersonOrPosition(Y9LoginUserHolder.getTenantId(), assignee).getData();
            model.setAssigneeId(assignee);
            ...
            HistoricVariableInstanceModel zhuBan = getZhuBanInfo(hai.getId(), year);
     ...
     * 设置办理人信息
     */
    private void setAssignee(HistoryProcessModel model, HistoricTaskInstanceModel hai, String taskId) {
        String assignee = hai.getAssignee();
        if (StringUtils.isNotBlank(assignee)) {
            setAssignedInfo(model, hai, taskId, assignee);
        } else {
            setUnassignedInfo(model, hai, taskId);
        }
    }
     ...
     * 设置办理人信息
     */
    private void setAssigneeInfo(HistoryProcessModel model, HistoricTaskInstanceModel hai, String year) {
        String assignee = hai.getAssignee();

        if (StringUtils.isNotBlank(assignee)) {
            setAssignedUserInfo(model, hai, year, assignee);
        } else {
            setUnassignedUserInfo(model, hai);
        }
    }
     ...
     * 设置候选人办理人信息
     */
    private void setCandidateAssigneeInfo(HistoryProcessModel model, List<IdentityLinkModel> candidateList) {
        String tenantId = Y9LoginUserHolder.getTenantId();
     ...
     * 设置描述信息
     */
    private void setDescriptionInfo(HistoryProcessModel model, HistoricTaskInstanceModel hai, String year) {
        ...
        try {
            handleDeleteMIExecutionDescription(model, hai, year);
        } catch (Exception e) {
     ...
     * 设置简单已分配用户信息
     */
    private void setSimpleAssignedUserInfo(HistoryProcessModel history, HistoricTaskInstanceModel hai, String year,
        String assignee) {
        try {
            String tenantId = Y9LoginUserHolder.getTenantId();
            String employeeName = "";
            OrgUnit employee = this.orgUnitApi.getPersonOrPosition(tenantId, assignee).getData();
            ...
            if (employee != null) {
                ...
                // 恢复待办，如不是办结人恢复，Owner有值，需显示Owner
                if (StringUtils.isNotBlank(ownerId)) {
                    OrgUnit ownerUser = this.orgUnitApi.getPersonOrPosition(tenantId, ownerId).getData();
     ...
     * 设置简单办理人信息
     */
    private void setSimpleAssigneeInfo(HistoryProcessModel history, HistoricTaskInstanceModel hai, String year) {
        String assignee = hai.getAssignee();

        if (StringUtils.isNotBlank(assignee)) {
            setSimpleAssignedUserInfo(history, hai, year, assignee);
        } else {
            setSimpleUnassignedUserInfo(history, hai);
        }
    }
        ...
        try {
            ...
            if (iList != null && !iList.isEmpty()) {
                ...
                for (IdentityLinkModel identityLink : iList) {
                    ...
                    OrgUnit ownerUser = this.orgUnitApi.getPersonOrPosition(tenantId, assigneeId).getData();
                    if (j < 5) {
     ...
     * 设置未分配用户信息
     */
    private void setUnassignedInfo(HistoryProcessModel model, HistoricTaskInstanceModel hai, String taskId) {
        try {
            String tenantId = Y9LoginUserHolder.getTenantId();
            List<IdentityLinkModel> iList = this.historicIdentityApi.getIdentityLinksForTask(taskId).getData();
            ...
            if (!candidateList.isEmpty()) {
                setCandidateAssigneeInfo(model, candidateList);
            }
            // 设置候选人列表
            List<Person> personList = new ArrayList<>();
            candidateList.forEach(candidate -> {
                try {
                    List<Person> personListTemp =
                        this.positionApi.listPersonsByPositionId(tenantId, candidate.getUserId()).getData();
                    personListTemp.forEach(p -> {
                        if (null != hai.getEndTime()) {
