...
public class DocumentServiceImpl implements DocumentService {
    ...
    public OpenDataModel add(String itemId, boolean mobile) {
        ...
        if (StringUtils.isNotBlank(itemId)) {
            ...
            ProcessDefinitionModel pdModel = repositoryApi.getLatestProcessDefinitionByKey(tenantId, processDefinitionKey).getData();
    ...
    public OpenDataModel edit(String itembox, String taskId, String processInstanceId, String itemId, boolean mobile) {
        ...
        if (itembox.equalsIgnoreCase(ItemBoxTypeEnum.TODO.getValue())) {
            ...
            List<TargetModel> nodeList = processDefinitionApi.getTargetNodes(tenantId, processDefinitionId, startNode).getData();
    ...
    public DocumentDetailModel editTodo(String taskId, boolean mobile) {
        DocumentDetailModel model = new DocumentDetailModel();
        String processSerialNumber, processDefinitionId, taskDefinitionKey, processDefinitionKey, activitiUser, itemId, starter;
        String tenantId = Y9LoginUserHolder.getTenantId();
        model.setMeeting(false);
        TaskModel task = taskApi.findById(tenantId, taskId).getData();
        String processInstanceId = task.getProcessInstanceId();
    ...
    public Y9Result<String> forwarding(String taskId, String sponsorHandle, String userChoice, String routeToTaskId, String sponsorGuid) {
        ...
        try {
            ...
            String multiInstance = processDefinitionApi.getNodeType(tenantId, task.getProcessDefinitionId(), routeToTaskId).getData();
            Map<String, Object> variables = CommonOpt.setVariables(orgUnitId, orgUnit.getName(), routeToTaskId, userList, multiInstance);
            ...
            if (SysVariables.PARALLEL.equals(multiInstance) && tooMuch) {
                ...
                asyncHandleService.forwarding(tenantId, orgUnit, processInstanceId, processParam, sponsorHandle, sponsorGuid, taskId, multiInstance, variables, userList);
    ...
    public OpenDataModel genDocumentModel(String itemId, String processDefinitionKey, String processDefinitionId, String taskDefinitionKey, boolean mobile, OpenDataModel model) {
        ...
        if (StringUtils.isBlank(processDefinitionId)) {
            processDefinitionId = repositoryApi.getLatestProcessDefinitionByKey(tenantId, processDefinitionKey).getData().getId();
        }
        // Y9表单
        String formIds = "";
        ...
        String formNames = "";
        if (mobile) {
            List<Y9FormItemMobileBind> eformTaskBinds = y9FormItemBindService.listByItemIdAndProcDefIdAndTaskDefKey4Mobile(itemId, processDefinitionId, taskDefinitionKey);
            model.setFormId("");
            model.setFormName("");
            model.setFormJson("");
        ...
        List<Y9FormItemBind> eformTaskBinds = y9FormItemBindService.listByItemIdAndProcDefIdAndTaskDefKey(itemId, processDefinitionId, taskDefinitionKey);
    ...
    @Override
    public DocumentDetailModel genTabModel(String itemId, String processDefinitionKey, String processDefinitionId, String taskDefinitionKey, boolean mobile, DocumentDetailModel model) {
        String tenantId = Y9LoginUserHolder.getTenantId();
        // Y9表单
        String showOtherFlag = "";
        if (mobile) {
            ...
            if (!eformTaskBinds.isEmpty()) {
                ...
                Y9Form y9Form = y9FormRepository.findById(eftb.getFormId()).orElse(null);
                assert y9Form != null;
                itemFormModel.setFormJson(y9Form.getFormJson());
                model.setFormList(List.of(itemFormModel));
            }
            return model;
        }
    ...
    public String getFirstItem() {
        try {
            ...
            String resourceId = "";
            List<Resource> list = positionResourceApi.listSubResources(tenantId, userId, AuthorityEnum.BROWSE, resourceId).getData();
            String url;
            for (Resource r : list) {
                url = r.getUrl();
    ...
    public String getFormIdByItemId(String itemId, String processDefinitionKey) {
        String formIds = "";
        String processDefinitionId = repositoryApi.getLatestProcessDefinitionByKey(Y9LoginUserHolder.getTenantId(), processDefinitionKey).getData().getId();
        List<Y9FormItemBind> eformTaskBinds = y9FormItemBindService.listByItemIdAndProcDefIdAndTaskDefKey(itemId, processDefinitionId, "");
        if (!eformTaskBinds.isEmpty()) {
            for (Y9FormItemBind eftb : eformTaskBinds) {
                formIds = Y9Util.genCustomStr(formIds, eftb.getFormId());
            }
    ...
    public List<OrgUnit> getUserChoice(String itemId, String processDefinitionId, String taskDefinitionKey, String processSerialNumber) {
        String tenantId = Y9LoginUserHolder.getTenantId();
        List<ItemPermission> list = itemPermissionService.listByItemIdAndProcessDefinitionIdAndTaskDefKeyExtra(itemId, processDefinitionId, taskDefinitionKey);
        ...
        for (ItemPermission o : list) {
            if (Objects.equals(o.getRoleType(), ItemPermissionEnum.DEPARTMENT.getValue()) || Objects.equals(o.getRoleType(), ItemPermissionEnum.POSITION.getValue()) || Objects.equals(o.getRoleType(), ItemPermissionEnum.USER.getValue())) {
                ...
                List<OrgUnit> deptList = roleApi.listOrgUnitsById(tenantId, o.getRoleId(), OrgTypeEnum.DEPARTMENT).getData();
                List<OrgUnit> personList = roleApi.listOrgUnitsById(tenantId, o.getRoleId(), OrgTypeEnum.POSITION).getData();
                ...
                List<OrgUnit> ouList = dynamicRoleMemberService.listByDynamicRoleIdAndProcessInstanceId(o.getRoleId(), processSerialNumber);
        ...
        try {
            ...
            for (Resource r : list) {
                ...
                if (spmApproveitem != null && spmApproveitem.getId() != null) {
                    ...
                    todoCount = processTodoApi.getTodoCountByUserIdAndProcessDefinitionKey(tenantId, userId, spmApproveitem.getWorkflowGuid()).getData();
                    model.setTodoCount((int) todoCount);
                    model.setAppIcon(StringUtils.isBlank(spmApproveitem.getIconData()) ? "" : spmApproveitem.getIconData());
    ...
    public OpenDataModel menuControl(String itemId, String processDefinitionId, String taskDefKey, String taskId, OpenDataModel model, String itembox) {
        ...
        for (int i = buttonOrders.length - 1; i >= 0; i--) {
            ...
            if (k == 15 && isButtonShow[15]) {
                List<TargetModel> taskNodes = processDefinitionApi.getNodes(tenantId, processDefinitionId, false).getData();
            ...
            for (String s : userChoices) {
                ...
                if (principalType == ItemPermissionEnum.POSITION.getValue()) {
                    ...
                    for (CustomGroupMember pTemp : list) {
                        OrgUnit orgUnit = orgUnitApi.getOrgUnitPersonOrPosition(tenantId, pTemp.getMemberId()).getData();
    ...
    public Y9Result<TargetModel> parserRouteToTaskId(String itemId, String processSerialNumber, String processDefinitionId, String taskDefKey, String taskId) {
        ...
        try {
            List<TargetModel> targetNodes = processDefinitionApi.getTargetNodes(tenantId, processDefinitionId, taskDefKey).getData();
            ...
            List<Y9FormItemBind> eformTaskBinds = y9FormItemBindService.listByItemIdAndProcDefIdAndTaskDefKey(itemId, processDefinitionId, taskDefKey);
            ...
            for (TargetModel targetNode : targetNodes) {
                ...
                boolean b = conditionParserApi.parser(tenantId, targetNode.getConditionExpression(), variables).getData();
                if (b) {
    ...
    public Y9Result<List<String>> parserUser(String itemId, String processDefinitionId, String routeToTaskId, String routeToTaskName, String processInstanceId, String multiInstance) {
        ...
        List<OrgUnit> orgUnitList = roleService.listPermUser4SUbmitTo(itemId, processDefinitionId, routeToTaskId, processInstanceId);
    ...
    public Y9Result<String> reposition(String taskId, String userChoice) {
        try {
            ...
            // 得到要发送节点的multiInstance，PARALLEL表示并行，SEQUENTIAL表示串行
            String multiInstance = processDefinitionApi.getNodeType(tenantId, task.getProcessDefinitionId(), task.getTaskDefinitionKey()).getData();
    ...
    public Y9Result<String> saveAndForwarding(String itemId, String processSerialNumber, String processDefinitionKey, String userChoice, String sponsorGuid, String routeToTaskId, Map<String, Object> variables) {
        ...
        if (!variables.isEmpty()) {
            ...
            if (variables.get("subProcessNum") != null) {// xxx并行子流程，userChoice只传了一个岗位id,根据subProcessNum，添加同一个岗位id,生成多个并行任务。
                String type = processDefinitionApi.getNodeType(tenantId, model.getProcessDefinitionId(), routeToTaskId).getData();
    ...
    public Y9Result<Object> saveAndSubmitTo(String itemId, String processSerialNumber) {
        ...
        try {
            ...
            String routeToTaskId = routeToTaskIdResult.getData().getTaskDefKey(), routeToTaskName = routeToTaskIdResult.getData().getTaskDefName();
            ...
            Map<String, Object> variables = CommonOpt.setVariables(userId, orgUnit.getName(), routeToTaskId, userList, multiInstance);
    ...
    public SignTaskConfigModel signTaskConfig(String itemId, String processDefinitionId, String taskDefinitionKey, String processSerialNumber) {
        ...
        try {
            ...
            String tenantId = Y9LoginUserHolder.getTenantId();
            String multiInstance = processDefinitionApi.getNodeType(tenantId, processDefinitionId, taskDefinitionKey).getData();
            if (SysVariables.COMMON.equals(multiInstance)) {
                ...
                if (itemTaskConf != null && itemTaskConf.getSignTask()) {
                    ...
                    if (processParam != null && StringUtils.isNotBlank(processParam.getProcessInstanceId())) {
                        List<HistoricTaskInstanceModel> hisTaskList = historictaskApi.findTaskByProcessInstanceIdOrByEndTimeAsc(tenantId, processParam.getProcessInstanceId(), "").getData();
    ...
    public Y9Result<String> start4Forwarding(String taskId, String routeToTaskId, String sponsorGuid, List<String> userList) {
        ...
        try {
            ...
            if (SysVariables.PARALLEL.equals(multiInstance) && tooMuch) {
                ...
                taskVariable.setText("true:" + num);
                taskVariableRepository.save(taskVariable);
                asyncHandleService.forwarding(tenantId, orgUnit, processInstanceId, processParam, "", sponsorGuid, taskId, multiInstance, variables, userList);
            } else if (SysVariables.SUBPROCESS.equals(multiInstance)) {
                Map<String, Object> vars = new HashMap<>(16);
                vars.put("parentTaskId", taskId);
                ...
                asyncHandleService.forwarding4Task(processInstanceId, processParam, "true", sponsorGuid, taskId, multiInstance, variables, userList);
            }
            return Y9Result.success(processInstanceId, "发送成功!");
        } catch (Exception e) {
            LOGGER.error("公文发送失败！");
    ...
    public StartProcessResultModel startProcess(String itemId, String processSerialNumber, String processDefinitionKey, String userIds) {
        ...
        try {
            ...
            vars = CommonOpt.setVariables(orgUnit.getId(), orgUnit.getName(), "", Arrays.asList(userIds.split(",")), processSerialNumber, "", vars);
            assert item != null;
            ProcessInstanceModel piModel = runtimeApi.startProcessInstanceByKey(tenantId, orgUnit.getId(), processDefinitionKey, item.getSystemName(), vars).getData();
    ...
    public Y9Result<Object> submitTo(String processSerialNumber, String taskId) {
        try {
            ...
            List<String> userList = userResult.getData();
            Map<String, Object> variables = CommonOpt.setVariables(userId, orgUnit.getName(), routeToTaskId, userList, multiInstance);
            String subProcessStr = variableApi.getVariableByProcessInstanceId(tenantId, processInstanceId, "subProcessNum").getData();
            ...
            asyncHandleService.forwarding4Task(processInstanceId, processParam, "true", "", taskId, multiInstance, variables, userList);
            return Y9Result.successMsg("提交成功");
        } catch (Exception e) {
            LOGGER.error("提交失败！", e);
        }
