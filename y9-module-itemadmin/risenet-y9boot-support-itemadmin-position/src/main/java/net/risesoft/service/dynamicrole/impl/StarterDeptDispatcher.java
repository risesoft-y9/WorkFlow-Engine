...
public class StarterDeptDispatcher extends AbstractDynamicRoleMember {
    ...
    public List<OrgUnit> getOrgUnitList(String processInstanceId) {
        ...
        if (StringUtils.isNotBlank(processInstanceId)) {
            ...
            if (StringUtils.isNotEmpty(userIdAndDeptId)) {
                ...
                List<OrgUnit> leaders = departmentApi.listDepartmentPropOrgUnits(tenantId, orgUnit.getParentId(), DepartmentPropCategoryEnum.DISPATCHER.getValue()).getData();
                orgUnitList.addAll(leaders);
            }
        } else {
            String positionId = Y9LoginUserHolder.getPositionId();
            Position position = positionManager.get(tenantId, positionId).getData();
            List<OrgUnit> leaders = departmentApi.listDepartmentPropOrgUnits(tenantId, position.getParentId(), DepartmentPropCategoryEnum.DISPATCHER.getValue()).getData();
            orgUnitList.addAll(leaders);
        }
        return orgUnitList;
    }
