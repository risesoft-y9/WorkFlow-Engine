...
public class OnApplicationReady implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private void creatApp() {
        try {
            ...
            List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, "itemAdmin");
            if (list.size() == 1) {
                Map<String, Object> smap = list.get(0);
                sql = "select * from Y9_COMMON_APP_STORE where CUSTOM_ID = ? and SYSTEM_ID = ?";
                List<Map<String, Object>> alist = jdbcTemplate.queryForList(sql, "banjian", smap.get("ID").toString());
                if (alist.isEmpty()) {
                    sql =
                    ...
                    jdbcTemplate.update(sql, Y9IdGenerator.genId(IdType.SNOWFLAKE), "办件", 0,
                        ...
                        smap.get("ID").toString(), Y9DateTimeUtils.formatCurrentDateTime(), "banjian", 2, 0, 0, 0, 1,
                        0);
                }
            }
        } catch (Exception e) {
    ...
    private void createSystem() {
        try {
            ...
            if (list.isEmpty()) {
                ...
                jdbcTemplate.update(sql, Y9IdGenerator.genId(IdType.SNOWFLAKE), "itemAdmin", "itemAdmin", "事项管理", 100,
                    1, 1, Y9DateTimeUtils.formatCurrentDateTime());
            }
        } catch (Exception e) {
