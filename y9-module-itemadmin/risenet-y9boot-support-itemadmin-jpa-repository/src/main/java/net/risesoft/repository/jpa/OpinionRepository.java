...
public interface OpinionRepository extends JpaRepository<Opinion, String>, JpaSpecificationExecutor<Opinion> {
     ...
     * @return
     */
    @Query("select count(*) from Opinion t where t.processSerialNumber = ?1 and t.opinionFrameMark=?2 and t.userId=?3  and (t.taskId is null or t.taskId='') order by t.createDate ASC")
